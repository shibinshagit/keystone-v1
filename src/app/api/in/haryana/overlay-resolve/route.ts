...
export async function POST(request: NextRequest) {
  try {
    ...
    const village = await HaryanaParcelService.resolveVillageOverlay({
      west: bounds.west,
      south: bounds.south,
      east: bounds.east,
      north: bounds.north,
    ...
    console.error("[Haryana Overlay Resolve API] Error:", error);
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to resolve Haryana overlay." },
      { status: 500 },
    );
  }
}
