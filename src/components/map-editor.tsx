...
} from "@/lib/engines/visual-analysis-engine";
...
const INDIA_PARCEL_GEOJSON_LABEL_ID = "india-parcels-geojson-label";
...
const WMS_RASTER_PAINT: mapboxgl.RasterPaint = {
  "raster-opacity": [
    "interpolate",
    ["linear"],
    ["zoom"],
...
export function MapEditor({
  ...
  const selectionHighlightTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (!mapInst.getLayer(INDIA_PARCEL_GEOJSON_LABEL_ID)) {
        mapInst.addLayer({
          id: INDIA_PARCEL_GEOJSON_LABEL_ID,
          type: "symbol",
          source: INDIA_PARCEL_GEOJSON_SOURCE_ID,
          layout: {
            "text-field": ["coalesce", ["get", "plotNo"], ""],
          ...
          paint: {
            "text-color": "rgba(59, 130, 246, 0.95)",
    ...
    const overlayKey = `${overlay.adapter.id}:${overlay.gisCode}:${overlay.overlayCodes || ""}`;
    const removeWmsSource = () => {
      if (mapInst.getLayer(INDIA_PARCEL_IMAGE_LAYER_ID)) {
    ...
    if (overlay.adapter.overlayRenderMode === "tiles") {
      const currentSourceType =
        mapInst.getStyle()?.sources?.[INDIA_PARCEL_IMAGE_SOURCE_ID]?.type || null;
      const tileParams = overlay.adapter.buildOverlayParams({
        bounds: overlay.bounds,
        gisCode: overlay.gisCode,
        overlayCodes: overlay.overlayCodes,
        viewportWidth: 256,
        viewportHeight: 256,
      });
      tileParams.set("srs", "EPSG:3857");
      tileParams.set("width", "256");
      tileParams.set("height", "256");
      tileParams.set("bbox", "{bbox-epsg-3857}");

      const tileUrl = `${overlay.adapter.wmsPath}?${tileParams
        .toString()
    ...
    const resolveIndiaOverlay = async () => {
      ...
      try {
        ...
        setIndiaOverlayState({
          ...
          overlayCodes: resolved.village.overlayCodes || null,
          bounds: overlayBounds,
          featureCollection: null,
        });

        if (
          resolved.adapter.overlaySourceType === "geojson" &&
          resolved.adapter.overlayFeaturesPath
        ) {
          const featuresResponse = await fetch(resolved.adapter.overlayFeaturesPath, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              bounds: overlayBounds,
  ...
  const handleMouseDown = useCallback(
    (e: mapboxgl.MapLayerMouseEvent) => {
      ...
      const draggableLayers = plots
        .flatMap((p) => [
          ...p.buildings.flatMap((b) =>
            b.floors.map((f) => `building-floor-fill-${f.id}-${b.id}`),
          ),
      ...
      if (layerId.startsWith("building-floor-fill-")) {
        ...
        for (const p of plots) {
          const matchedBuilding = p.buildings.find((b) =>
            layerId.endsWith(`-${b.id}`),
          );
          if (matchedBuilding) {
      ...
      const targetPlot = plots.find(
        (p) =>
          ...
          (type === "ParkingArea" &&
            p.parkingAreas.some((pa) => pa.id === id)) ||
          (type === "UtilityArea" && p.utilityAreas.some((u) => u.id === id)) ||
          (type === "BuildableArea" &&
            p.buildableAreas.some((b) => b.id === id)),
      );
  ...
  const handleMapClick = useCallback(
    async (e: mapboxgl.MapLayerMouseEvent) => {
          ...
          fetch(wmsUrl.toString())
            ...
            .then((payload) => {
              ...
              if (text.includes("ServiceException")) {
                ...
                actions.setBhuvanData(
                  "Error: Invalid query parameters or layer not available here.",
                  false,
                );
                return;
              }
              ...
              if (
                text.includes("<body></body>") ||
                text.trim() === "" ||
                text.length < 50
              ) {
                actions.setBhuvanData(
                  "No specific thematic data found exactly at this point.",
                  false,
                );
              } else {
      ...
      if (
        ...
        const indiaParcelAdapters = getIndiaParcelClientAdapters(coordinates);
            ...
            for (const indiaParcelAdapter of indiaParcelAdapters) {
              ...
              for (const candidate of clickCandidates) {
                ...
                if (indiaParcel?.plotId && indiaParcel.gisCode) {
                  resolvedParcelAdapter = indiaParcelAdapter;
                  break;
                }
              }
      ...
      if (drawingState.isDrawing) {
        ...
        if (
          typeof drawingState.objectType !== "string" ||
          (drawingState.objectType.toLowerCase() !== "road" &&
            drawingPoints.length > 2)
        ) {
          const firstPoint = drawingPoints[0];
          const clickPoint: LngLatLike = {
            lng: e.lngLat.lng,
            lat: e.lngLat.lat,
          };
          const firstMapPoint: LngLatLike = {
            lng: firstPoint[0],
            lat: firstPoint[1],
          };
          const pixelDist = map.current
            ?.project(clickPoint)
            .dist(map.current.project(firstMapPoint));
        ...
        if (
          typeof currentDrawingState.objectType !== "string" ||
          currentDrawingState.objectType.toLowerCase() !== "select"
        )
          return;

        const allMapLayers = mapInst.getStyle().layers.map((l) => l.id);
        const clickableLayers = plots
          .flatMap((p) => [
            `plot-base-${p.id}`,
            ...p.buildings.flatMap((b) =>
              b.floors.map((f) => `building-floor-fill-${f.id}-${b.id}`),
        ...
        if (features && features.length > 0) {
          ...
          if (layerId.startsWith("building-floor-fill-")) {
            for (const plot of plots) {
              const matchedBuilding = plot.buildings.find((b) =>
                layerId.endsWith(`-${b.id}`),
              );
              if (matchedBuilding) {
    ...
    (e: mapboxgl.MapLayerMouseEvent) => {
      ...
      if (drawingState.isDrawing) {
        ...
        if (drawingPoints.length > 0) {
          ...
          const outlineSource = map.current.getSource(
            DRAWING_OUTLINE_SOURCE_ID,
          ) as GeoJSONSource;
          const labelsSource = map.current.getSource(
            DRAWING_LABELS_SOURCE_ID,
          ) as GeoJSONSource;
          const roadFillSource = map.current.getSource(
            "drawing-road-fill",
          ) as GeoJSONSource;

          if (outlineSource)
            outlineSource.setData(turf.featureCollection([line]));
          if (labelsSource)
            labelsSource.setData(createDistanceLabels(previewPoints));

          if (drawingState.objectType === "Road" && roadFillSource) {
            const buffered = turf.buffer(line, drawingState.roadWidth / 2, {
              units: "meters",
            });
            if (buffered)
              roadFillSource.setData(turf.featureCollection([buffered]));
          }

          if (drawingPoints.length > 2) {
            const firstPoint = drawingPoints[0];
            const hoverPoint: LngLatLike = {
              lng: e.lngLat.lng,
              lat: e.lngLat.lat,
            };
            const firstMapPoint: LngLatLike = {
        ...
        if (isSelectMode) {
          ...
          if (hoverableLayers.length > 0) {
            ...
            map.current.getCanvas().style.cursor =
              features && features.length > 0 ? "pointer" : "default";
          } else {
    ...
    mapInstance.on("load", () => {
      ...
      if (ctx) {
        ...
        mapInstance.addImage(
          "wind-arrow",
          ctx.getImageData(0, 0, arrowSize, arrowSize),
        );
      }
  ...
  useEffect(() => {
    ...
    if (activeProject?.location && typeof activeProject.location === "object") {
      const { lat, lng } = activeProject.location as {
        lat: number;
        lng: number;
      };
      if (lat && lng) {
  ...
  useEffect(() => {
    ...
    const detectRoads = () => {
      ...
      plotsRef.current.forEach((plot) => {
        ...
        if (
          plot.roadAccessSides !== undefined ||
          pendingRoadDetections.current.has(plot.id)
        )
          return;

        pendingRoadDetections.current.add(plot.id);
        console.log(
          `[Road Debug] Detecting roads for Plot ${plot.id} using Source Query...`,
        );
        ...
        const searchPoly = turf.buffer(plot.geometry as any, 0.05, {
          units: "kilometers",
        });
        ...
        if (plots[0] === plot) {
          ...
          console.log(
            `[Road Debug] Style Sources:`,
            Object.keys(style.sources),
          );
          if ((style as any).imports) {
            console.log(
              `[Road Debug] Style Imports:`,
              (style as any).imports.map((i: any) => i.id),
            );
          }
          console.log(
            `[Road Debug] Found ${roadFeatures.length} total roads in viewport.`,
          );
          console.log(
            `[Road Debug] Found ${relevantRoads.length} roads near plot.`,
          );
        }
        ...
        const determineAccessSides = (features: any[]) => {
          ...
          const nLine = turf.lineString([
            [minX, maxY],
            [maxX, maxY],
          ]);
          const sLine = turf.lineString([
            [minX, minY],
            [maxX, minY],
          ]);
          const eLine = turf.lineString([
            [maxX, minY],
            [maxX, maxY],
          ]);
          const wLine = turf.lineString([
            [minX, minY],
            [minX, maxY],
          ]);
          ...
          features.forEach((rf) => {
            if (turf.booleanIntersects(rf as any, nZone as any))
              accessSides.add("N");
            if (turf.booleanIntersects(rf as any, sZone as any))
              accessSides.add("S");
            if (turf.booleanIntersects(rf as any, eZone as any))
              accessSides.add("E");
            if (turf.booleanIntersects(rf as any, wZone as any))
              accessSides.add("W");
          });
        ...
        if (relevantRoads.length > 0) {
          ...
          if (plots[0] === plot)
            console.log(`[Road Debug] Detected Sides (Local):`, newSides);
    ...
    return () => {
      map.current?.off("idle", detectRoads);
    };
  }, [isMapLoaded, actions]);

  const [districtNameHint, setDistrictNameHint] = useState<
    string | undefined
  >();
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${plotLng},${plotLat}.json?access_token=${mapboxToken}&types=district,place,locality,neighborhood,postcode&country=IN`,
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) =>
        console.error("Reverse geocoding error for district hint:", err),
      );
  ...
  useEffect(() => {
    ...
    if (activeBhuvanLayer) {
      ...
      mapInst.addSource(sourceId, {
        type: "raster",
        tiles: [`${tileProxyBase}&bbox={bbox-epsg-3857}`],
        tileSize: 256,
      });

      const firstSymbolId = mapInst
        .getStyle()
  ...
  useEffect(() => {
    ...
    getStoreState().activeBhuvanOpacity,
    activeBhuvanLayer,
    isMapLoaded,
    styleLoaded,
    getStoreState,
  ...
  useEffect(() => {
    ...
    amenities.forEach((amenity: Amenity) => {
      ...
      if (amenity.category === "transit")
        ...
        color = "#FF9800"; // Orange
      else if (amenity.category === "college")
        color = "#FF5722"; // Deep Orange
      else if (amenity.category === "hospital")
        color = "#F44336"; // Red
      ...
      else if (amenity.category === "restaurant")
        color = "#FFEB3B"; // Yellow
      else if (amenity.category === "atm")
        color = "#009688"; // Teal
  ...
  const cleanupOverlays = useCallback(() => {
    ...
    if (
      windStreamlineLayer.current &&
      map.current.getLayer("wind-streamlines")
    ) {
  ...
  const resetBuildingColors = (forcedColor?: string) => {
    ...
    plots.forEach((plot) => {
      plot.buildings.forEach((building) => {
        const colorToApply =
          forcedColor || getBuildingColor(building.intendedUse);
        ...
        building.floors.forEach((floor) => {
          ...
          if (map.current!.getLayer(layerId)) {
            if (forcedColor) {
              ...
              const isBasementOrParking =
                (floor.level !== undefined && floor.level < 0) ||
                (floor.type || "").toLowerCase() === "parking";
              const usePattern =
                !uiState.ghostMode &&
                !isBasementOrParking &&
                analysisMode === "none";
              if (usePattern) {
                ...
                const userOpacity =
                  building.opacity !== undefined ? building.opacity : 1.0;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      console.log(
        `[MAP EDITOR] Running ${analysisMode} on ${allBuildings.length} buildings...`,
      );
      ...
      if (plots.length > 0 && plots[0].geometry) {
        try {
          ...
          console.log(
            "[MAP EDITOR] Weather data:",
            weatherData.isLive ? "🟢 LIVE/ERA5" : "🟡 ESTIMATED",
            "for",
            solarDate.toDateString(),
          );
        } catch (err) {
      ...
      const wallFeatures = await runWallAnalysis(
        allBuildings,
        allBuildings,
        analysisMode,
        solarDate,
      ...
      console.log("[MAP EDITOR] Wall Analysis complete, features:", {
        count: wallFeatures.features.length,
      });
      ...
      if (map.current) {
        if (map.current.getSource(wallSourceId)) {
          (
            map.current.getSource(wallSourceId) as mapboxgl.GeoJSONSource
          ).setData(wallFeatures);
        } else {
      ...
      if (map.current && plots.length > 0) {
        try {
          ...
          if (analysisMode === "wind") {
            actions.updateSimulationResults({
              wind: {
                compliantArea: stats.compliantArea,
                avgSpeed: stats.avgValue,
              },
            });
          ...
          if (["energy", "mobility", "resilience"].includes(analysisMode)) {
            console.log(
              `[MAP EDITOR] ${analysisMode.toUpperCase()} Stats:`,
              stats,
            );
          }
          ...
          if (buildingResults.size > 0 && map.current) {
            ...
            buildingResults.forEach((result: any, buildingId: string) => {
              ...
              const analysisColor =
                result.roofColor || result.color || "#eeeeee";
              const buildingHeight =
                building.height ||
                building.floors?.reduce(
                  (s: number, f: any) => s + f.height,
                  0,
                ) ||
                10;
            ...
            if (map.current.getSource(roofSourceId)) {
              (
                map.current.getSource(roofSourceId) as mapboxgl.GeoJSONSource
              ).setData(roofCollection as any);
            } else {
            ...
            console.log(
              `[MAP EDITOR] Applied analysis roof overlay to ${roofFeatures.length} buildings`,
            );
          }
          ...
          if (groundPoints && groundPoints.features.length > 0) {
            if (map.current.getSource(heatmapId)) {
              (map.current.getSource(heatmapId) as GeoJSONSource).setData(
                groundPoints,
              );
            } else {
            ...
            if (!map.current.getLayer(heatmapId)) {
              ...
              if (
                ...
                colorRamp = [
                  ...
                  ["heatmap-density"],
                  0,
                  "rgba(239, 68, 68, 0)", // Transparent red
                  0.2,
                  "#ef4444", // red-500 (Stagnant/Shady/Dark)
                  0.4,
                ...
                colorRamp = [
                  ...
                  0.2,
                  "#3b82f6", // blue-500 (Cool/Efficient)
                  0.4,
                  "#06b6d4", // cyan-500
                  0.6,
                ...
                colorRamp = [
                  ...
                  0.2,
                  "#22c55e", // green-500 (Low traffic)
                  0.4,
                  "#84cc16", // lime-500
                  0.6,
                  "#f59e0b", // amber-500 (Moderate)
                  0.8,
                  "#f97316", // orange-500
                  1,
                  "#ef4444", // red-500 (High traffic)
                ];
                ...
                colorRamp = [
                  ...
                  "#84cc16", // lime-500
                  0.6,
                  "#eab308", // yellow-500 (Moderate risk)
                  0.8,
                  "#ef4444", // red-500 (High risk)
                  1,
                  "#991b1b", // red-800 (Extreme risk)
                ];
              } else {
              ...
              if (styleLayers) {
                const firstBuildingLayer = styleLayers.find(
                  (l: any) =>
              ...
              map.current.addLayer(
                ...
                insertBeforeId,
              ); // Place strictly below buildings
            }
            ...
            if (analysisMode === "wind") {
              ...
              const windDir =
                weatherData && weatherData.hourly
                  ? weatherData.hourly.windDirection[currentHour]
                  : 45;

              if (!windStreamlineLayer.current) {
                windStreamlineLayer.current = new WindStreamlineLayer(
                  "wind-streamlines",
                );
                ...
                if (!map.current.getLayer("wind-streamlines")) {
                  addLayerSafely(
                    map.current,
                    windStreamlineLayer.current as any,
                    LABELS_LAYER_ID,
                  );
                }
  ...
  useEffect(() => {
    ...
    if (isSimulatorEnabled) {
      ...
      const { azimuth, altitude } = getSunPosition(
        solarDate,
        center.lat,
        center.lng,
      );
  ...
  const buildingProps = useMemo(
    () =>
      plots
        .flatMap((p) =>
          p.buildings.map(
            (b) => `${b.id}-${b.opacity}-${b.height}-${b.numFloors}`,
          ),
        )
        .join(","),
    [plots],
    ...
    if (drawingState.isDrawing) {
      drawingPoints.forEach((point, index) => {
        ...
        const marker = new mapboxgl.Marker({
          color: isFirstPoint ? FIRST_POINT_COLOR : primaryColor,
        })
          .setLngLat(point as LngLatLike)
          .addTo(mapInstance);
        markers.current.push(marker);
      });
      ...
      const labelsSource = mapInstance.getSource(
        DRAWING_LABELS_SOURCE_ID,
      ) as GeoJSONSource;
      const roadFillSource = mapInstance.getSource(
        "drawing-road-fill",
      ) as GeoJSONSource;
      ...
      if (outlineSource) {
        ...
        mapInstance.addSource(DRAWING_OUTLINE_SOURCE_ID, {
          type: "geojson",
          data: outlineData,
        });
        mapInstance.addLayer({
      ...
      if (labelsSource) {
        ...
        mapInstance.addSource(DRAWING_LABELS_SOURCE_ID, {
          type: "geojson",
          data: labelsData,
        });
        mapInstance.addLayer({
      ...
      if (roadFillSource) {
        ...
        mapInstance.addSource("drawing-road-fill", {
          type: "geojson",
          data: roadFillData,
        });
        mapInstance.addLayer({
      ...
      if (mapInstance.getLayer(DRAWING_LABELS_LAYER_ID))
        mapInstance.removeLayer(DRAWING_LABELS_LAYER_ID);
      if (mapInstance.getSource(DRAWING_LABELS_SOURCE_ID))
        mapInstance.removeSource(DRAWING_LABELS_SOURCE_ID);
    }
  ...
  useEffect(() => {
    if (plots.length > 0) {
      console.log(
        `[MapEditor] ðŸ•µï¸ Plots Data Updated. Count: ${plots.length}`,
      );
      const p0 = plots[0];
  ...
  useEffect(() => {
    ...
    const featureData = turf.feature(
  ...
  useEffect(() => {
    ...
    const anyComponentVisible = Object.values(componentVisibility).some(
      (v) => v,
    );
        ...
        paint: {
          ...
          "text-opacity": false
            ...
            : [
                ...
                ["boolean", ["feature-state", "hover"], false],
                1,
                ["==", ["get", "linkedId"], "SELECTED_ID_PLACEHOLDER"],
                1,
                0,
              ],
        },
    ...
    if (mapInstance.getLayer(LABELS_LAYER_ID)) {
      mapInstance.setPaintProperty(
        ...
        false
          ? 0
          : ["case", ["==", ["get", "linkedId"], hoveredId || ""], 1, 0],
      );
    }
    ...
    plots.forEach((plot) => {
      ...
      plot.greenAreas.forEach((area) => {
        ...
        if (!mapInstance.getLayer(areaId)) {
          const isSelected =
            selectedObjectId &&
            selectedObjectId.id === area.id &&
            selectedObjectId.type === "GreenArea";
          ...
          mapInstance.setPaintProperty(
            areaId,
            "fill-color",
            isSelected ? selectionColor : "#4ade80",
          );
          mapInstance.setPaintProperty(
            areaId,
            "fill-opacity",
          ...
          mapInstance.setPaintProperty(
            areaId,
            "fill-outline-color",
            isSelected ? selectionColor : "#22c55e",
          );
        }
      });
      ...
      plot.buildableAreas.forEach((area) => {
        ...
        const isSelected =
          selectedObjectId &&
          selectedObjectId.id === area.id &&
          selectedObjectId.type === "BuildableArea";
        const selectionColor = "#00fbff";
        ...
        if (!mapInstance.getLayer(areaId)) {
          ...
          mapInstance.setPaintProperty(
            areaId,
            "fill-color",
            isSelected ? selectionColor : "#f59e0b",
          );
          mapInstance.setPaintProperty(
            areaId,
            "fill-opacity",
          ...
          mapInstance.setPaintProperty(
            areaId,
            "fill-outline-color",
            isSelected ? selectionColor : "#b45309",
          );
      ...
      plot.parkingAreas.forEach((area) => {
        ...
        const areaData: Feature<Polygon> = {
          type: "Feature",
          geometry:
            (area.geometry as any).type === "Feature"
              ? (area.geometry as any).geometry
              : area.geometry,
          properties: {
            id: area.id,
            name: area.name,
        ...
        if (!mapInstance.getLayer(areaId)) {
          ...
          if (!isBasement) {
            ...
            mapInstance.setPaintProperty(
              areaId,
              "fill-color",
              isSelected ? selectionColor : "#607D8B",
            );
            mapInstance.setPaintProperty(
              areaId,
              "fill-opacity",
            ...
            mapInstance.setPaintProperty(
              areaId,
              "fill-outline-color",
              isSelected ? selectionColor : "#455A64",
            );
          }
        }
      });
      ...
      utilitiesToRender.forEach((u) => {
        ...
        const featureData = {
          type: "Feature",
          geometry:
            (u.geometry as any).type === "Feature"
              ? (u.geometry as any).geometry
              : u.geometry,
          properties: {
            id: u.id,
            name: u.name,
        ...
        else if (typeStr.includes("wtp") || typeStr.includes("water"))
          color = "#2196F3";
        ...
        else if (typeStr.includes("dg") || typeStr.includes("transformer"))
          color = "#3A4F2E"; // Olive Green
        else if (typeStr.includes("road")) color = "#546E7A";
        else if (typeStr.includes("solar"))
          color = "#1A237E"; // Solar Indigo
        else if (typeStr.includes("waste") || typeStr.includes("owc"))
          color = "#8D6E63"; // Brown
            ...
            mapInstance.addLayer(
              {
                ...
                paint: {
                  ...
                  "fill-extrusion-height":
                    u.type === ("Solar PV" as any) ? 3.5 : 2.5,
                  "fill-extrusion-opacity": isSelected ? 0.9 : 0.7,
                  "fill-extrusion-base":
                    u.type === ("Solar PV" as any) ? 3.0 : 0,
                },
                metadata: {
                  id: u.id,
                  type: "UtilityArea",
          ...
          if (isRoad) {
            if (mapInstance.getLayer(areaId)?.type === "fill") {
              ...
              mapInstance.setPaintProperty(
                areaId,
                "fill-outline-color",
                isSelected ? selectionColor : "#2c3e50",
              );
            }
          } else {
      ...
      plot.buildings.forEach((building) => {
        ...
        if (building.floors && building.floors.length > 0) {
          const basementFloors = building.floors.filter(
            (f) =>
              (f.level !== undefined && f.level < 0) || f.type === "Parking",
          );
          const superstructureFloors = building.floors.filter(
            (f) =>
              !((f.level !== undefined && f.level < 0) || f.type === "Parking"),
          );
          ...
          const basementFloorsCalc = building.floors.filter(
            (f) =>
              (f.level !== undefined && f.level < 0) ||
              (f.type || "").toLowerCase() === "parking",
          );
          let totalBasementHeight = basementFloorsCalc.reduce(
            (sum, f) => sum + f.height,
            0,
          );
          ...
          if (totalBasementHeight === 0 && building.id.includes("-tower")) {
            ...
            const siblingPodium = plot.buildings.find(
              (b) => b.id === `${baseId}-podium`,
            );
            if (siblingPodium) {
              const siblingBasementFloors = siblingPodium.floors.filter(
                (f) =>
                  (f.level !== undefined && f.level < 0) ||
                  (f.type || "").toLowerCase() === "parking",
              );
              totalBasementHeight = siblingBasementFloors.reduce(
                (sum, f) => sum + f.height,
                0,
              );
            }
          ...
          const superstructureFloorsCalc = building.floors.filter(
            (f) =>
              !(
                (f.level !== undefined && f.level < 0) ||
                (f.type || "").toLowerCase() === "parking"
          ...
          const superstructureHeight = superstructureFloorsCalc.reduce(
            (sum, f) => sum + (f.height || 3),
            0,
          );
          const superstructureFloorsCalcFiltered = building.floors.filter(
            (f) =>
              !(
                (f.level !== undefined && f.level < 0) ||
                (f.type || "").toLowerCase() === "parking"
          ...
          const superstructureHeightFinal =
            superstructureFloorsCalcFiltered.reduce(
              (sum, f) => sum + (f.height || 3),
              0,
            );
          const visualBuildingTop =
            (building.baseHeight || 0) +
            (shouldLiftForBasements ? totalBasementHeight : 0) +
            superstructureHeightFinal;
          const effectiveBase =
            (building.baseHeight || 0) +
            (shouldLiftForBasements ? totalBasementHeight : 0);
            ...
            building.internalUtilities.forEach((util: UtilityArea) => {
              ...
              if (util.type === "Electrical") {
                ...
                if (building.internalsVisible === false) {
                  ...
                  utilOpacity = componentVisibility.electrical
                    ? 1.0
                    : anyComponentVisible
                      ? 0.0
                      : uiState.ghostMode
                ...
                if (building.internalsVisible === false) {
                  ...
                  utilOpacity = componentVisibility.hvac
                    ? 1.0
                    : anyComponentVisible
                      ? 0.0
                      : uiState.ghostMode
                ...
                if (building.internalsVisible === false) {
                  ...
                  utilOpacity = componentVisibility.solar
                    ? 0.9
                    : anyComponentVisible
                      ? 0.0
                      : uiState.ghostMode
                ...
                if (building.internalsVisible === false) {
                  ...
                  utilOpacity = componentVisibility.ev
                    ? 1.0
                    : anyComponentVisible
                      ? 0.0
                      : uiState.ghostMode
                ...
                if (b1) {
                  utilBase =
                    (building.baseHeight || 0) +
                    (shouldLiftForBasements
                      ? totalBasementHeight - b1.height
                      : -b1.height);
          ...
          floorsToRender.forEach((f) => {
            floorDict[f.id] = {
              baseHeight: fdCurrentBase,
              height: fdCurrentBase + f.height,
            };
          ...
          if (building.units && floorsToRender) {
            ...
            building.units.forEach((unit: Unit) => {
              ...
              const unitTop = Math.max(
                fBounds.baseHeight,
                fBounds.height - SLAB_GAP,
              );
          ...
          floorsToRender.forEach((floor, fIndex) => {
            ...
            try {
              const buffered = turf.buffer(building.geometry, -0.0005, {
                units: "kilometers",
              });
            ...
            let slabSource = mapInstance.getSource(
              slabLayerId,
            ) as GeoJSONSource;
            if (slabSource) slabSource.setData(slabGeo);
            else
              mapInstance.addSource(slabLayerId, {
                type: "geojson",
                data: slabGeo,
              });
            ...
            let finalColor: any = isBuildingSelected
              ? selectionColor
              : intendedColor || floor.color || "#cccccc";
            ...
            if (fSource) fSource.setData(floorGeo);
              ...
              mapInstance.addSource(floorLayerId, {
                type: "geojson",
                data: floorGeo,
              });
              ...
              if (usePattern) {
                ...
                try {
                  mapInstance.setPaintProperty(
                    floorLayerId,
                    "fill-extrusion-color",
                    isBuildingSelected ? selectionColor : ["get", "color"],
                  );
                } catch (e) {}
              }
            }
    ...
    plotsRendering.forEach((plot) => {
      ...
      if (geometryType === "Feature" && (geometryToRender as any).geometry) {
        console.log(
          `[MapEditor] Normalizing Feature to Geometry for Plot ${plotId}`,
        );
        geometryToRender = (geometryToRender as any).geometry;
        geometryType = geometryToRender.type;
      } else {
        console.log(
          `[MapEditor] No normalization needed for Plot ${plotId}. Type: ${geometryType}`,
        );
      }
      ...
      try {
        if (
          ((geometryType as string) === "Polygon" ||
            (geometryType as string) === "MultiPolygon") &&
          plot.setback > 0
        ) {
          setbackPolygon = turf.buffer(plot.geometry as any, -plot.setback, {
            units: "meters",
          });
        }
      } catch (e) {
      ...
      if (
        !validNormalizedGeometry ||
        typeof validNormalizedGeometry !== "object" ||
        !validNormalizedGeometry.type ||
        !(validNormalizedGeometry as any).coordinates
      ) {
        console.warn(
          `[MapEditor] âŒ Invalid Geometry Object for Plot ${plotId}`,
          validNormalizedGeometry,
        );
      }
      ...
      if (sourceBase) {
        ...
        if (dataToRender)
          mapInstance.addSource(plotBaseSourceId, {
            type: "geojson",
            data: dataToRender,
          });
      }
      ...
      let sourceSetback = mapInstance.getSource(
        plotSetbackSourceId,
      ) as GeoJSONSource;
      if (sourceSetback) sourceSetback.setData(setbackPolygon || dataToRender);
      else if (dataToRender)
        mapInstance.addSource(plotSetbackSourceId, {
          type: "geojson",
          data: setbackPolygon || dataToRender,
        });
      ...
      if (sourceLabel) sourceLabel.setData(labelData as any);
        ...
        mapInstance.addSource(labelSourceId, {
          type: "geojson",
      ...
      console.log(
        `[MapEditor] Rendering gates for plot ${plot.id}`,
        plot.entries,
      );
    ...
    let buildingSource = mapInstance.getSource(
      allBuildingsSourceId,
    ) as GeoJSONSource;
    // @ts-ignore
    ...
    if (buildingSource) buildingSource.setData(buildingCollection);
      ...
      mapInstance.addSource(allBuildingsSourceId, {
        type: "geojson",
    ...
    if (currentStyle && currentStyle.layers) {
      currentStyle.layers.forEach((layer) => {
        const layerId = layer.id;
        const isManagedByPlots =
          layerId.startsWith("plot-") ||
          layerId.startsWith("building-") ||
          layerId.startsWith("building-floor-fill-") ||
          layerId.startsWith("building-slab-") ||
          layerId.startsWith("building-front-") ||
          layerId.startsWith("green-") ||
          layerId.startsWith("parking-") ||
          layerId.startsWith("buildable-") ||
          layerId.startsWith("util-") ||
          layerId.startsWith("utility-area-") ||
          layerId.startsWith("core-") ||
          layerId.startsWith("unit-") ||
          layerId.startsWith("units-") ||
          layerId.startsWith("cores-") ||
          layerId.startsWith("electrical-") ||
          layerId.startsWith("hvac-") ||
          layerId.startsWith("gates-");

        if (
          isManagedByPlots &&
          !renderedIds.has(layerId) &&
          layerId !== LABELS_LAYER_ID
        ) {
    ...
    if (currentStyle && currentStyle.sources) {
      Object.keys(currentStyle.sources).forEach((sourceId) => {
        const isManagedByPlots =
          sourceId.startsWith("plot-") ||
          sourceId.startsWith("building-") ||
          sourceId.startsWith("building-floor-fill-") ||
          sourceId.startsWith("building-slab-") ||
          sourceId.startsWith("building-front-") ||
          sourceId.startsWith("green-") ||
          sourceId.startsWith("parking-") ||
          sourceId.startsWith("buildable-") ||
          sourceId.startsWith("util-") ||
          sourceId.startsWith("utility-area-") ||
          sourceId.startsWith("core-") ||
          sourceId.startsWith("unit-") ||
          sourceId.startsWith("units-") ||
          sourceId.startsWith("cores-") ||
          sourceId.startsWith("electrical-") ||
          sourceId.startsWith("hvac-") ||
          sourceId.startsWith("gates-");

        if (
          isManagedByPlots &&
          !renderedIds.has(sourceId) &&
          sourceId !== LABELS_SOURCE_ID
        ) {
          const style = mapInstance.getStyle();
          const isSourceInUse = style?.layers?.some(
            (layer) => (layer as any).source === sourceId,
          );
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: mapboxgl.MapMouseEvent) => {
      ...
      if (
        typeof ds.objectType === "string" &&
        ds.objectType.toLowerCase() === "rotate"
      ) {
        popup.remove();
        return;
      }

      const internalsVisible =
        cv.units || cv.cores || cv.electrical || cv.hvac || us.ghostMode;
      ...
      if (features.length > 0) {
        ...
        if (f.layer?.id === "all-buildings-hit-layer") {
          ...
          const typeLabel =
            props.type ||
            (f.layer?.id.startsWith("parking-area-") ? "Parking" : "Utility");

          const actualArea = Number(props.targetArea || props.area) || 0;
          const areaLabel = actualArea
            ? `Footprint: ${actualArea.toFixed(1)} m²`
            : "";
          const targetAreaLabel = "";

          const capacityLabel = props.capacity
            ? `<div class="text-xs text-neutral-600 mt-0.5">Capacity: ${props.capacity} cars</div>`
            : "";
          ...
          const actualArea = props.targetArea
            ? Number(props.targetArea).toFixed(1)
            : planarArea(f).toFixed(1);
          html = `
      ...
      <div className="absolute top-4 right-14 z-10 bg-background/90 backdrop-blur rounded-md border shadow-sm p-0.5 flex items-center gap-0.5">
        ...
        >
          ...
          {mapStyleMode === "terrain" && (
            <>
              <ImageIcon className="w-3.5 h-3.5" /> Terrain
            </>
          )}
        </button>
