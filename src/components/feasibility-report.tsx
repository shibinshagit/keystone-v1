...
const PageBreak = () => (
  <div className="page-break" style={{ breakAfter: "page" }} />
);

const SH = ({ children }: { children: React.ReactNode }) => (
  <h2 className="text-base font-bold bg-slate-100 p-2 mb-3 border-l-4 border-slate-800 uppercase tracking-wide">
    {children}
  </h2>
);
const SH2 = ({
  ...
  <h3
    className={`text-sm font-bold text-slate-700 mt-5 mb-2 border-b border-slate-200 pb-1 ${className}`}
  >
    {children}
  </h3>
);
...
const TH = ({
  ...
  <th
    className={`p-1.5 border border-slate-600 bg-slate-800 text-white text-[10px] font-semibold text-left ${className}`}
    colSpan={colSpan}
    rowSpan={rowSpan}
  >
    {children}
...
const TD = ({
  ...
  <td
    className={`p-1.5 border border-slate-300 text-[10px] ${className}`}
    colSpan={colSpan}
    rowSpan={rowSpan}
  >
    {children}
...
export function FeasibilityReport({
  ...
  const far =
    plot.far || plot.regulation?.geometry?.floor_area_ratio?.value || 1.0;
  const maxCov =
    plot.maxCoverage ||
  ...
  const achievedFAR =
    metrics?.achievedFAR ?? (builtUp && plotArea ? builtUp / plotArea : 0);
  const gcPct =
    metrics?.groundCoveragePct ??
  ...
  const hasRWH =
    greenItems.find((i: any) => i.label.includes("Rainwater"))?.status ===
    "pass";
  const hasSolar =
    greenItems.find((i: any) => i.label.includes("Solar"))?.status === "pass";
  const hasSTP =
    greenItems.find(
      (i: any) =>
        i.label.includes("STP") || i.label.includes("Water Recycling"),
    )?.status === "pass";
  ...
  const timeRange = sim
    ? `${Math.round(sim.time_p10)} – ${Math.round(sim.time_p90)} months`
    : tl
      ? `${fmt(tl.total_months, 0)} months`
      : "30 months";

  // per-building core breakdowns
  const buildingCores = (plot.buildings || []).map((b) => {
    const floors =
      b.numFloors || Math.ceil(b.height / (b.typicalFloorHeight || 3));
    let useType: "Residential" | "Commercial" | "Institutional" = "Residential";
    if (String(b.intendedUse) === "Commercial") useType = "Commercial";
    const core = calculateBuildingCoreAndCirculation({
  ...
  return (
    <div
      className="bg-white text-black text-[10.5px] font-sans leading-relaxed print:bg-white"
    ...
    >
      ...
      <div className="report-page flex flex-col items-center justify-center min-h-[250mm] text-center">
        ...
        <p className="text-xs tracking-[.25em] text-slate-500 uppercase mb-2">
          Automated Feasibility Report
        </p>
        <h1 className="text-3xl font-black text-slate-900 uppercase leading-tight mb-1">
          {project.name || "Untitled Project"}
        </h1>
        <p className="text-lg text-slate-500 mb-6">
          {project.intendedUse || "Residential"} Development
        </p>
        <div className="w-16 h-0.5 bg-blue-600 mb-6" />
        <p>
          Plot Area: {fmt(plotArea)} sq.m &nbsp;|&nbsp; {towers} Tower(s)
          &nbsp;|&nbsp; {totalUnits} Units
        </p>
        <p>
          Location:{" "}
          {typeof plot.location === "object"
            ? "Coordinates Defined"
            : String(plot.location || project.location || "N/A")}
        </p>
        ...
        <div className="mt-auto pt-8 pb-4">
          <p className="text-[9px] text-slate-400">
            Generated by Keystone Engine — Confidential
          </p>
        </div>
      </div>
      <PageBreak />
      ...
      <div className="report-page">
        ...
        <table className="w-full border-collapse border border-slate-300 mb-4">
          <tbody>
            {[
              ...
              [
                "Site Area",
                `${fmt(plotArea)} sq.m (${fmt(plotArea / 4046.86, 2)} Acres)`,
              ],
              [
                "Development Type",
                `${project.intendedUse || "High-Rise Residential"}`,
              ],
              [
                "Total Units",
                `${totalUnits} Apartments (${Object.keys(units).join(", ") || "Mixed"})`,
              ],
              [
                "Configuration",
                `${towers} Towers, ${Math.ceil(totalUnits / (towers * Math.max(1, maxFloors)))} Units per Floor`,
              ],
              [
                "Building Height",
                `${fmt(maxHeight, 1)}m (G+${maxFloors} floors)`,
              ],
              [
                "Total Levels",
                `${totalLevels} (B${maxBasements} to Floor ${maxFloors})`,
              ],
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px]">
          {(() => {
            ...
            if (plotArea > 5000)
              strengths.push(
                `Large plot area (${fmt(plotArea)} sq.m) offers strong development scale.`,
              );
            else
              strengths.push(
                `Compact plot area (${fmt(plotArea)} sq.m) suitable for boutique ${project.intendedUse?.toLowerCase() || "residential"} development.`,
              );

            if ((achievedFAR || 0) >= far * 0.9)
              strengths.push(
                `Optimal FAR utilization (${fmt(achievedFAR, 2)} of ${far}).`,
              );
            else
              strengths.push(
                `High permissible FAR (${far}) available for development.`,
              );

            const entriesCount = plot.entries?.length || 0;
            if (entriesCount > 1)
              strengths.push(
                `Multiple accesses (${entriesCount} gates) ease traffic flow.`,
              );

            // Weaknesses
            if (entriesCount <= 1)
              weaknesses.push(
                "Single point of access may cause traffic bottlenecks.",
              );

            const avgSetback = generationParams?.setback ?? plot?.setback ?? 0;
            if (avgSetback >= 6)
              weaknesses.push(
                `Stringent setback requirements (≥${avgSetback}m) reduce available footprint.`,
              );
            else
              weaknesses.push(
                "Required setbacks constrain the ground coverage area.",
              );

            if (plotArea < 3000)
              weaknesses.push(
                "Small plot size constrains large-scale amenity provisioning.",
              );

            // Opportunities
            opportunities.push(
              `Demand for premium ${project.intendedUse?.toLowerCase() || "residential"} spaces in the micro-market.`,
            );
            opportunities.push(
              "Scope for green building certification (IGBC/GRIHA) to unlock premium pricing.",
            );
            if (far - (achievedFAR || 0) > 0.5)
              opportunities.push(
                `Significant unutilized FAR (${fmt(far - (achievedFAR || 0), 2)}) presents future expansion potential.`,
              );

            // Threats
            if (parkProv < parkReq)
              threats.push(
                `Parking shortfall (Provided: ${parkProv}, Required: ${parkReq} ECS) may pose regulatory hurdles.`,
              );
            else
              threats.push(
                `Strict parking requirements (${parkReq} ECS) mandate extensive basement construction.`,
              );

            threats.push(
              `Ground coverage limitation (Max ${maxCov}%) restricts low-rise sprawl.`,
            );

            const heightLimit = plot.regulation?.geometry?.max_height?.value;
            if (heightLimit)
              threats.push(
                `Strict height limitation (${heightLimit}m) caps vertical revenue potential.`,
              );
            else
              threats.push(
                "Market slowdown or fluctuations in material costs.",
              );
      ...
      <div className="report-page">
        ...
        <table className="w-full border-collapse text-[9px] mb-4">
          <thead>
            <tr>
              <TH className="w-1/3 text-left">Parameter</TH>
          ...
          <tbody>
            {(() => {
              ...
              const accessStr = [
                entryNames ||
                  `${entries.length > 0 ? entries.length + " gate(s)" : "Gate"}`,
              ...
              const findNearest = (keywords: string[]) => {
                ...
                hits.sort(
                  (a: any, b: any) => (a.distance ?? 99) - (b.distance ?? 99),
                );
                const h = hits[0];
                const dist =
                  h.distance != null
              ...
              const connLines = [
                highway
                  ? `• ${highway.name || "Nearest Highway"}: ${highway.dist ?? "nearby"}`
                  : null,
                metro
                  ? `• ${metro.name || "Metro Station"}: ${metro.dist ?? "nearby"}`
                  : null,
                airport
                  ? `• ${airport.name || "Airport"}: ${airport.dist ?? "nearby"}`
                  : null,
              ].filter(Boolean);
              const connStr =
              ...
              const rows: [string, string][] = [
                [
                  "Plot Area",
                  `${fmt(plotArea)} sq.m (${Math.round(Math.sqrt(plotArea))}m × ${Math.round(Math.sqrt(plotArea))}m approx.)`,
                ],
                [
                  "Location",
                  typeof plot.location === "object"
                    ? "Coordinates Defined"
                    : String(
                        plot.location || project.location || "[Sector/Area]",
                      ),
                ],
                [
                  "Zoning",
                  `${project.intendedUse || "Residential"} (Group Housing permitted)`,
                ],
              ...
              return rows.map(([k, v], i) => (
                <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                  ...
                  <TD className="whitespace-pre-line leading-tight py-1">
                    {v}
                  </TD>
                </tr>
              ));
        ...
        <table className="w-full border-collapse text-[9px] mb-1">
          ...
          <tbody>
            {[
              ...
              [
                "Tower Size",
                `~${Math.round(builtUp / totalLevels / towers)} sq.m each`,
              ],
              [
                "Floors",
                `B${maxBasements} + Ground + ${maxFloors} Upper = ${totalLevels} levels`,
              ],
        ...
        <p className="text-[9px] font-medium text-green-700 mb-4 bg-green-50 p-1 border border-green-200 inline-block">
          <span className="font-bold">Carpet Efficiency:</span>{" "}
          {carpetEff.toFixed(1)}% (Industry: 55-65%) ✔
        </p>
        ...
        <table className="w-full border-collapse text-[9px] mb-1 text-center">
          <thead>
            <tr>
              ...
              <TH>Count</TH>
              <TH>%</TH>
              <TH className="text-left">Target Segment</TH>
            </tr>
          </thead>
          <tbody>
        ...
        <div className="bg-slate-50 border border-slate-200 p-2 text-[9px] mb-4 mt-1">
          <strong className="text-slate-800 inline-block mb-1">
            Unit Mix Assessment:{" "}
            <span className="text-green-700">
              ✔{" "}
              {Object.keys(units).length <= 1
                ? "Homogeneous, market-appropriate"
                : "Diverse, market-appropriate"}
            </span>
          </strong>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <strong className="block text-slate-700 mb-1 border-b pb-1">
              Parking{" "}
              {plot.regulation?.location?.includes("Haryana")
                ? "(Code 7.1(2))"
                : ""}
            </strong>
            <table className="w-full border-collapse text-[9px] mb-1">
              <thead>
                <tr>
                  <TH className="text-left w-1/2">Type</TH>
                  <TH>Spaces</TH>
                  <TH>Compliance</TH>
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <TD
                    className={`text-center font-bold border-l border-slate-200 ${parkProv >= parkReq ? "text-green-700" : "text-red-600"}`}
                  >
                    {parkProv >= parkReq
                ...
                <tr className="bg-slate-50 text-slate-500">
                  ...
                  <TD className="text-center">
                    {parkProv -
                      Math.floor(parkProv * 0.39) -
                      Math.floor(parkProv * 0.38)}{" "}
                    ECS
                  </TD>
                  <TD className="border-l border-slate-200">&nbsp;</TD>
                </tr>
            ...
            <p
              className={`text-[9px] font-medium p-1 border text-center mt-1 ${parkProv >= parkReq ? "text-green-700 bg-green-50 border-green-200" : "text-red-700 bg-red-50 border-red-200"}`}
            >
              Assessment:{" "}
              {parkProv >= parkReq
      ...
      <div className="report-page">
        ...
        <ul className="list-disc pl-4 space-y-1 mb-3">
          <li>
            <strong>Zoning:</strong> {project.intendedUse || "Residential"}{" "}
            (Group Housing permissible)
          </li>
          <li>
            <strong>Development Plan:</strong> As per{" "}
            {regLabel(plot.regulation?.location)}
          </li>
          <li>
            <strong>FAR Permitted:</strong> Base {far}, achievable up to 3.5
            with incentives
          </li>
          <li>
            <strong>Ground Coverage:</strong> Maximum {maxCov}%{" "}
            {plot.regulation?.location?.includes("Haryana")
              ? "(Code 6.3(3)(i)(b))"
              : ""}
          </li>
        ...
        <div className="mb-4 text-[10px]">
          ...
          <strong className="block text-slate-800 mb-1">
            Available Building Zone:
          </strong>
          <ul className="list-disc pl-4 space-y-0.5 mb-2">
            <li>
              Net buildable area derived after deducting {plot.setback || 10}m
              setbacks from all sides.
            </li>
            <li>
              Available footprint zone: ~
              {fmt(plot.developmentStats?.maxBuildableArea || plotArea * 0.6)}{" "}
              sq.m
            </li>
          </ul>
          ...
          <ul className="list-disc pl-4 space-y-0.5">
            <li>
              {maxCov}% of {fmt(plotArea)} sq.m ={" "}
              {fmt((maxCov / 100) * plotArea)} sq.m
            </li>
          </ul>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-1 text-[10px]">
              <li>
                <strong>Based on load:</strong>{" "}
                {fmt((plot.buildings?.[0]?.numFloors || maxFloors) * 305)}{" "}
                tonnes per tower
              </li>
              <li>
                <strong>Option 1:</strong> Deep raft foundation (500mm) @ 8-10m
                depth
              </li>
              <li>
                <strong>Option 2:</strong> Pile foundation (800mm dia., 20-25m
                depth, 40-50 piles/tower)
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-1 text-[10px]">
              <li>
                <strong>Best facades:</strong> North and East (minimize heat
                gain)
              </li>
              <li>
                <strong>Avoid:</strong> West-facing large openings
              </li>
              <li>
                <strong>Recommended:</strong> Deep balconies on south/west
              </li>
            </ul>
          </div>
        </div>
        <ul className="list-disc pl-4 space-y-1 mb-5">
          ...
          <li>
            {plot.regulation?.location?.includes("Haryana")
              ? "Haryana Fire Services Act 2009"
              : "State Fire Services Act / NBC Part 4"}
          </li>
          <li>RERA (Real Estate Regulation and Development Act) 2016</li>
          <li>Environmental clearance norms (Code Chapter 12)</li>
        </ul>
        ...
        <table className="w-full border-collapse mb-4 text-[11px]">
          ...
          <tbody>
            <tr>
              <TD className="font-semibold bg-slate-50">FAR</TD>
              <TD>
                Max {far} (base), up to{" "}
                {plot.regulation?.geometry?.max_far_with_incentive?.value ??
                  3.5}{" "}
                with incentives
              </TD>
              <TD>{fmt(achievedFAR, 2)}</TD>
              <TD>
                {achievedFAR <= far ? (
                  <>
                    <Check /> Yes
                  ...
                  <span className="text-orange-600 font-bold">
                    ⚠ Above base FAR — incentive required
                  </span>
                ) : (
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Ground Coverage</TD>
              <TD>Max {maxCov}%</TD>
              <TD>{fmt(gcPct, 1)}%</TD>
              <TD>
                {gcPct <= maxCov ? (
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Height</TD>
              <TD>
                {plot.regulation?.geometry?.max_building_height?.value
                  ? `Max ${plot.regulation.geometry.max_building_height.value}m`
                  : "Unrestricted (with NOC)"}
              </TD>
              <TD>{fmt(maxHeight, 1)}m</TD>
              <TD>
                {!plot.regulation?.geometry?.max_building_height?.value ||
                maxHeight <=
                  plot.regulation?.geometry?.max_building_height?.value +
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Basement</TD>
              <TD>Up to 4 levels</TD>
              <TD>
                {plot?.parkingAreas?.filter((p) => p.type === "Basement")
                  ?.length || 2}{" "}
                levels
              </TD>
              <TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Parking</TD>
              <TD>1.5 ECS per unit ({parkReq} required)</TD>
              <TD>{parkProv} ECS provided</TD>
              <TD>
                {parkProv >= parkReq ? (
                  <>
                    <Check /> Yes
                  ...
                  <span className="text-orange-600 font-bold">
                    ⚠ Pending ({parkReq - parkProv} short)
                  </span>
                )}
              </TD>
            </tr>
            <tr>
              <TD className="font-semibold bg-slate-50">Unit sizes</TD>
              <TD>Min. NBC standards (≥30 sq.m net)</TD>
              <TD>
                {metrics?.sellableArea
                  ? fmt(
                      metrics.sellableArea /
                        Math.max(1, metrics.totalUnits || 1),
                    )
                  : 110}{" "}
                sq.m avg
              </TD>
              <TD>
                {(metrics?.sellableArea
                  ? metrics.sellableArea / Math.max(1, metrics.totalUnits || 1)
                  : 110) >= 30 ? (
                  <>
                    <Check /> Yes
                  </>
            ...
            {(() => {
              ...
              const totalLifts = core
                ? core.passLiftCount +
                  core.fireLiftCount +
                  core.serviceLiftCount
                : 3;

              const frontReq =
                plot?.regulation?.geometry?.front_setback?.value ??
                generationParams?.frontSetback ??
                generationParams?.setback ??
                6;
              const rearReq =
                plot?.regulation?.geometry?.rear_setback?.value ??
                generationParams?.rearSetback ??
                generationParams?.setback ??
                3;
              const sideReq =
                plot?.regulation?.geometry?.side_setback?.value ??
                generationParams?.sideSetback ??
                generationParams?.setback ??
                3;
              ...
              return (
                <>
                  <tr>
                    <TD className="font-semibold bg-slate-50">Staircases</TD>
                    <TD>
                      {stairCount} per tower, {stairWidth}m min width
                    </TD>
                    <TD>
                      {stairCount} × {stairWidth}m
                    </TD>
                    <TD>
                      <Check /> Yes
                    </TD>
                  ...
                  <tr>
                    <TD className="font-semibold bg-slate-50">Lifts</TD>
                    <TD>
                      Required for &gt;15m;{" "}
                      {maxHeight > 60 ? "≥4" : maxHeight > 30 ? "≥3" : "≥2"} per
                      tower
                    </TD>
                    <TD>{totalLifts} lifts/tower</TD>
                    <TD>
                      {totalLifts >=
                      (maxHeight > 60 ? 4 : maxHeight > 30 ? 3 : 2) ? (
                        <Check />
                      ) : (
                  ...
                  <tr>
                    <TD className="font-semibold bg-slate-50">Setbacks</TD>
                    <TD>
                      F:{frontReq}m / Rear:{rearReq}m / Side:{sideReq}m
                      (regulation)
                    </TD>
                    <TD>
                      F:{frontProv}m / Rear:{rearProv}m / Side:{sideProv}m
                    </TD>
                    <TD>
                      {setbackOk ? (
                        <>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Basement Parking</TD>
              <TD>Max roof 1.5m above ground</TD>
              <TD>1.5m</TD>
              <TD>
                <Check /> Yes
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Fire Safety</TD>
              <TD>As per NBC/Fire Act</TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">RWH</TD>
              <TD>
                Mandatory for &gt;500 sq.m (
                {Math.max(2, Math.ceil(plotArea / 5000))} bores min.)
              </TD>
              <TD>
                {(() => {
                  const item = greenItems.find((i: any) =>
                    i.label.includes("Rainwater"),
                  );
                  return (
                    item?.detail ||
                    (hasRWH
                      ? `${Math.max(2, Math.ceil(plotArea / 5000))} bores provided`
                      : "Not Provided")
                  );
                })()}
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Solar</TD>
              <TD>1% of connected load</TD>
              <TD>
                {(() => {
                  const item = greenItems.find((i: any) =>
                    i.label.includes("Solar"),
                  );
                  return (
                    item?.detail ||
                    (hasSolar
                      ? `~${Math.round(totalUnits * 0.5)} kWp installed`
                      : "Not Provided")
                  );
                })()}
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">ECBC</TD>
              <TD>Mandatory for certain buildings</TD>
              <TD>Applicable</TD>
              <TD>
                <Check /> Yes
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Structural Safety</TD>
              <TD>High-rise requirements</TD>
              <TD>Full compliance</TD>
              <TD>
                <Check /> Yes
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Sanitation</TD>
              <TD>Min. facilities</TD>
              <TD>As per norms</TD>
              <TD>
                <Check /> Yes
              </TD>
            ...
            <tr>
              <TD className="font-semibold bg-slate-50">Zero discharge</TD>
              <TD>Mandatory for GH</TD>
              <TD>{hasSTP ? "STP Provided" : "Not Provided"}</TD>
              <TD>
                {hasSTP ? (
                  <>
                    <Check /> Yes
        ...
        {metrics?.compliance?.bylawItems &&
          metrics.compliance.bylawItems.length > 0 && (
            <>
              ...
              <table className="w-full border-collapse mb-4">
                ...
                <tbody>
                  {metrics.compliance.bylawItems.slice(0, 12).map((it, i) => (
                    <tr key={i}>
                      <TD className="font-semibold bg-slate-50">{it.label}</TD>
                      <TD>{it.detail || "—"}</TD>
                      <TD>
                        {it.status === "pass" ? (
                          <Check />
      ...
      <div className="report-page">
        ...
        {[
          {
            part: "Part 3: Development Control Rules & General Building Requirements",
            items: [
              "FAR and coverage within limits",
              "Setbacks as per height",
              "Unit sizes meet minimum standards",
          ...
          },
          {
            ...
            items: [
              "Fire exits within 30m travel distance",
              "Fire staircases: 2 per tower (enclosed, pressurized)",
              "Wet riser system in fire shaft",
              "Fire extinguishers on all floors",
              "Smoke detectors in lobbies and corridors",
              "Emergency lighting and signage",
              "Refuge areas on alternate floors (for >24m height)",
              "Fire alarm system integrated",
              "Sprinkler system in basements",
            ],
          },
          {
            part: "Part 5: Building Materials",
            items: [
              "All materials as per IS specifications",
              "Concrete: M30 (columns), M25 (beams/slabs)",
              "Steel: Fe500D (earthquake-resistant)",
              "Bricks/blocks: As per IS 1077/2185",
              "DPC: As per Code 10.1.5",
            ],
          },
          {
            part: "Part 6: Structural Design",
            items: [
              "Loads as per IS 875 (Parts 1-5)",
              "Seismic design: IS 1893:2016 (Zone IV)",
              "Concrete design: IS 456:2000",
              "Steel design: IS 800:2007",
              "Foundation design: IS 6403:1981",
              "Wind load analysis: IS 875 Part 3",
            ],
          },
          {
            part: "Part 7: Construction Practices & Safety",
            items: [
              "Scaffolding as per IS 3696",
              "Formwork as per IS 14687",
              "Concrete quality control: IS 456 Annexure",
          ...
          },
          {
            ...
            items: [
              "Electrical: IS 732, IS 1646",
              "Plumbing: IS 1172, IS 2065",
              "HVAC: IS 3315 series",
              "Lifts: IS 14665, IS 15259",
            ],
          },
          {
            part: "Part 9: Plumbing Services",
            items: [
              "Water supply: IS 1172",
              "Drainage: Two-pipe system (Code 11.1)",
              "Sanitary fixtures: IS 2556",
              "STP capacity: 25,000 lpd",
            ],
          },
        ].map((sec, i) => (
          <div key={i} className="mb-3 no-break">
            <strong className="text-[10px] block text-slate-800 mb-1">
              {sec.part}
            </strong>
        ...
        <SH2>
          {plot.regulation?.location?.includes("Haryana")
            ? "Fire Safety Compliance (Haryana Fire Services Act 2009)"
            : "Fire Safety Compliance"}
        </SH2>
        <strong className="text-[12px] block text-slate-800 mb-2 mt-1">
          Fire NOC Requirements (
          {maxHeight > 60
            ? "High-Rise buildings >60m"
            : maxHeight > 30
              ? "Buildings 30-60m height"
              : "Buildings 15-30m height"}
          )
        </strong>
        <table className="w-full border-collapse">
          ...
          <tbody>
            {(() => {
              ...
              const fireLifts =
                buildingCores[0]?.core?.fireLiftCount ?? (mid ? 1 : 0);
              ...
              const rows: [string, string, string][] = [
                [
                  "Fire staircases",
                  `2 per tower, ${mid ? "pressurized" : "enclosed"}`,
                  `${towers * 2} ${mid ? "pressurized" : "enclosed"}`,
                ],
                [
                  "Staircase width",
                  `Min ${hi ? "2.0" : "1.5"}m clear`,
                  `${hi ? "2.0" : "1.5"}m`,
                ],
                ["Travel distance", "Max 30m from unit door", "<30m"],
                [
                  "Fire lift",
                  mid ? "≥1 per tower (mandatory >15m)" : "Not required",
                  mid ? `${towers * Math.max(1, fireLifts)} provided` : "N/A",
                ],
                [
                  "Wet riser",
                  `100mm dia., every floor${mid ? " + hose reel" : ""}`,
                  "Provided",
                ],
                [
                  "Fire extinguishers",
                  "2 per floor minimum",
                  `${Math.max(2, Math.ceil((totalUnits / maxFloors) * 2))} per floor`,
                ],
                [
                  "Smoke detectors",
                  mid ? "Addressable, zone-controlled" : "All common areas",
                  "Full coverage",
                ],
                [
                  "Fire alarm",
                  mid ? "Auto addressable + PA system" : "Addressable system",
                  "Integrated + PA",
                ],
                [
                  "Sprinklers",
                  mid ? "All floors + basements + podium" : "All basements",
                  "Full coverage",
                ],
                ...(mid
                  ? ([
                      [
                        "Fire check lobby",
                        "Required (height >24m)",
                        "Provided per staircase",
                      ] as [string, string, string],
                      [
                        "Refuge area",
                        `Every 7 floors (${refugeFloors} levels)`,
                        `${refugeFloors} refuge floors`,
                      ] as [string, string, string],
                      [
                        "Emergency lighting",
                        "6 hrs min. battery backup",
                        "LED + UPS",
                      ] as [string, string, string],
                      [
                        "Terrace access",
                        "Min 1 staircase to terrace",
                        "Provided",
                      ] as [string, string, string],
                    ] as [string, string, string][])
                  : []),
                ...(hi
                  ? ([
                      [
                        ...
                        "Staircase & lobby +50Pa",
                        "Designed",
                      ] as [string, string, string],
                      [
                        "Firefighter intercom",
                        "All floors + fire command",
                        "Integrated",
                      ] as [string, string, string],
                    ] as [string, string, string][])
                  : []),
              ...
              return rows.map(([a, b, c], i) => (
                <tr key={i}>
                  <TD className="font-semibold bg-slate-50">{a}</TD>
                  <TD>{b}</TD>
                  <TD>{c}</TD>
                </tr>
              ));
      ...
      <div className="report-page">
        ...
        <strong className="text-[11px] block text-slate-800 mb-2 mt-4">
          Category {builtUp < 20000 ? "A" : builtUp < 50000 ? "B" : "C"} -
          ...
          <tbody>
            {[
              ...
              [
                ...
                "Min 1 bore per 5,000 sq.m",
                `${Math.max(2, Math.ceil(plotArea / 5000))} bores`,
              ],
              ...
              [
                "Green Cover",
                "1 tree per 80 sq.m",
                `${Math.ceil(plotArea / 80)} trees (${fmt(plotArea, 0)}/80)`,
              ],
              [
                "Compensatory Plantation",
                "3:1 for any trees cut",
                "N/A (no trees)",
              ],
              [
                "Dust Control",
                "Screens/barricading during construction",
                "Yes",
              ],
            ].map(([a, b, c], i) => (
              <tr key={i}>
        ...
        <strong className="text-[11px] block text-slate-800 mb-2">
          Required Approvals
        </strong>
        <table className="w-full border-collapse">
          <thead>
          ...
          <tbody>
            {[
              ...
              [
                "PWD (B&R)",
                "Road cutting (services)",
                "If required",
                "15 days",
              ],
              [
                "Electricity Dept",
                "Connection & load sanction",
                "Yes",
                "45 days",
              ],
              ...
              <tr key={i}>
                <TD className="font-semibold bg-slate-50">{auth}</TD>
                <TD>{clear}</TD>
                <TD>{app}</TD>
                <TD>{time}</TD>
              </tr>
      ...
      <div className="report-page">
        ...
        <ol className="list-decimal pl-5 space-y-1 mb-6 text-[11px]">
          <li>
            <strong>Maximize Unit Count:</strong> Achieve {totalUnits} units
            while maintaining quality
          </li>
          <li>
            <strong>Efficient Circulation:</strong> Minimize core-to-usable area
            ratio ({carpetEff.toFixed(1)}% efficiency)
          </li>
          <li>
            <strong>Natural Light:</strong> All rooms with external openings
          </li>
          <li>
            <strong>Cross Ventilation:</strong> Through units wherever possible
          </li>
          <li>
            <strong>Privacy:</strong> Limited units per floor
          </li>
          <li>
            <strong>Flexibility:</strong> Adaptable layouts for future
            modifications
          </li>
          <li>
            <strong>Sustainability:</strong> Green building features integrated
          </li>
          <li>
            <strong>Code Compliance:</strong> 100% adherence to regulations
          </li>
        </ol>

        <SH2>Space Distribution</SH2>
        <p className="mb-2">
          <strong>Total Site:</strong> {fmt(plotArea)} sq.m
        </p>
        <table className="w-full border-collapse mb-6 text-[10px]">
          ...
          <tbody>
            {(() => {
              const bFootprint =
                plot.buildings?.reduce((sum, b) => sum + (b.area || 0), 0) ||
                plotArea * 0.25;
              ...
              const hardScape = plotArea * 0.2; // 20% for drives/plazas

              const maxPermissibleGC = plotArea * 0.35; // Typically 35% max GC
              const remainingUnbuilt =
                plotArea - Math.max(bFootprint, towerFootprint * towers);
              ...
              const percent = (val: number) =>
                ((val / plotArea) * 100).toFixed(1) + "%";

              return (
                <>
                  <tr className="font-semibold bg-slate-50">
                    <TD>Building Footprint</TD>
                    <TD>{fmt(bFootprint)}</TD>
                    <TD>{percent(bFootprint)}</TD>
                  </tr>
                  {plot.buildings?.map((b, i) => (
                    <tr key={i}>
                      ...
                      <TD>{fmt(b.area)}</TD>
                      <TD>{percent(b.area || 0)}</TD>
                    </tr>
                  ))}
                  <tr className="font-semibold bg-slate-50">
                    <TD>Hard Landscaping</TD>
                    <TD>{fmt(hardScape)}</TD>
                    <TD>{percent(hardScape)}</TD>
                  </tr>
                  <tr>
                    <TD className="pl-6 text-slate-600">
                      - Entrance plaza & Parking ramp
                    </TD>
                    <TD>{fmt(hardScape * 0.5)}</TD>
                    <TD>{percent(hardScape * 0.5)}</TD>
                  </tr>
                  <tr>
                    <TD className="pl-6 text-slate-600">- Pathways & drives</TD>
                    <TD>{fmt(hardScape * 0.5)}</TD>
                    <TD>{percent(hardScape * 0.5)}</TD>
                  </tr>
                  ...
                  <tr className="font-semibold bg-slate-50">
                    <TD>Amenities (Ground inside towers)</TD>
                    <TD>{fmt(amenities)}</TD>
                    <TD>{percent(amenities)}</TD>
                  </tr>

                  <tr className="font-semibold bg-slate-50">
                    <TD>Setbacks (Unusable)</TD>
                    <TD>{fmt(Math.max(0, setbacks))}</TD>
                    <TD>{percent(Math.max(0, setbacks))}</TD>
                  </tr>
                  <tr className="font-bold bg-slate-200 border-t-2 border-slate-400">
                    <TD>Total</TD>
                    <TD>{fmt(plotArea)}</TD>
                    <TD>100.0%</TD>
                  </tr>
        ...
        {buildingCores.length > 0 && (
          <>
              ...
              <tbody>
                {buildingCores.map((bc, i) => (
                  <tr key={i}>
                    ...
                    <TD>
                      {bc.core.passLiftCount +
                        bc.core.fireLiftCount +
                        bc.core.serviceLiftCount}
                    </TD>
            ...
            <div className="mb-4">
              <strong className="text-[11px] block text-slate-800 mb-1">
                A. Core Configuration
              </strong>
              <p className="text-[10px] mb-2">
                <strong>Core Size:</strong>{" "}
                {buildingCores[0]?.core?.totalCoreAreaPerFloor
                  ? Math.ceil(
                      Math.sqrt(buildingCores[0].core.totalCoreAreaPerFloor),
                    )
                  : 8}
                m ×{" "}
                {buildingCores[0]?.core?.totalCoreAreaPerFloor
                  ...
                  : 10}
                m = {fmt(buildingCores[0]?.core?.totalCoreAreaPerFloor || 80)}{" "}
                sq.m per tower
                <br />
                Core Components (Enhanced for luxury apartments)
              </p>
                  ...
                  <table className="w-full border-collapse text-[10px]">
                    ...
                    <tbody>
                      <tr className="bg-slate-50 font-semibold">
                        <TD>Lifts</TD>
                        <TD>
                          {totalLifts} total ({c.passLiftCount} pass
                          {c.fireLiftCount ? ` + ${c.fireLiftCount} fire` : ""}
                          {c.serviceLiftCount
                            ? ` + ${c.serviceLiftCount} service`
                            : ""}
                          )
                        </TD>
                        <TD>{fmt(c.liftArea, 1)}</TD>
                      </tr>
                      {c.passLiftCount > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                          ...
                          <TD>
                            {c.passLiftCount} nos. (~
                            {fmt(c.passLiftArea / c.passLiftCount, 2)} sq.m
                            each)
                          </TD>
                          <TD>{fmt(c.passLiftArea, 1)}</TD>
                        </tr>
                      )}
                      {c.fireLiftCount > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">- Fire lift</TD>
                          <TD>{c.fireLiftCount} nos. (4.5 sq.m each)</TD>
                          <TD>{fmt(c.fireLiftArea, 1)}</TD>
                        </tr>
                      )}
                      {c.serviceLiftCount > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Service lift
                          </TD>
                          <TD>{c.serviceLiftCount} nos. (4.5 sq.m each)</TD>
                          <TD>{fmt(c.serviceLiftArea, 1)}</TD>
                        </tr>
                      )}
                      {c.stretcherLiftCount > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Stretcher lift
                          </TD>
                          <TD>{c.stretcherLiftCount} nos. (5.5 sq.m each)</TD>
                          <TD>{fmt(c.stretcherLiftArea, 1)}</TD>
                        </tr>
                      )}

                      <tr className="bg-slate-50 font-semibold">
                        <TD>Staircases</TD>
                        <TD>{c.stairCount} enclosed fire stairs</TD>
                        <TD>{fmt(c.stairArea, 1)}</TD>
                      </tr>
                      {[...Array(c.stairCount)].map((_, i) => (
                        <tr key={i}>
                          ...
                          <TD>
                            ~{fmt(c.stairArea / c.stairCount, 0)} sq.m (1.5m min
                            clear width)
                          </TD>
                          <TD>{fmt(c.stairArea / c.stairCount, 1)}</TD>
                        </tr>
                      ))}
                      ...
                      <tr className="bg-slate-50 font-semibold">
                        <TD>Shafts & Ducts</TD>
                        <TD> </TD>
                        <TD>{fmt(c.totalShaftArea, 1)}</TD>
                      </tr>
                      {c.plumbingShaftArea > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Plumbing shafts
                          </TD>
                          <TD>Per unit group</TD>
                          <TD>{fmt(c.plumbingShaftArea, 2)}</TD>
                        </tr>
                      )}
                      {c.electricalShaftArea > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Electrical riser
                          </TD>
                          <TD>0.8m × 0.75m</TD>
                          <TD>{fmt(c.electricalShaftArea, 2)}</TD>
                        </tr>
                      )}
                      {c.fireRiserArea > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">- Fire riser</TD>
                          <TD>1.0m × 0.5m</TD>
                          <TD>{fmt(c.fireRiserArea, 2)}</TD>
                        </tr>
                      )}
                      {c.garbageShaftArea > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Garbage shaft
                          </TD>
                          <TD>0.6m × 1.2m</TD>
                          <TD>{fmt(c.garbageShaftArea, 2)}</TD>
                        </tr>
                      )}
                      {c.hvacShaftArea > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">- HVAC shaft</TD>
                          <TD>As required</TD>
                          <TD>{fmt(c.hvacShaftArea, 2)}</TD>
                        </tr>
                      )}

                      <tr className="bg-slate-50 font-semibold">
                        <TD>Lobbies & Circulation</TD>
                        <TD> </TD>
                        <TD>{fmt(c.totalCirculationAreaPerFloor, 1)}</TD>
                      </tr>
                      <tr>
                        <TD className="pl-6 text-slate-600">- Lift lobby</TD>
                        <TD>
                          {totalLifts} × {fmt(c.liftLobbyArea / totalLifts, 1)}{" "}
                          sq.m each
                        </TD>
                        <TD>{fmt(c.liftLobbyArea, 1)}</TD>
                      </tr>
                      <tr>
                        <TD className="pl-6 text-slate-600">
                          - Common corridor
                        ...
                        <TD>Min 1.5m wide</TD>
                        <TD>{fmt(c.corridorArea, 1)}</TD>
                      </tr>

                      {(c.fireCheckLobbyArea > 0 ||
                        c.refugeAreaPerFloor > 0) && (
                        <tr className="bg-slate-50 font-semibold">
                          <TD>Fire Provisions</TD>
                          <TD> </TD>
                          <TD>
                            {fmt(
                              c.fireCheckLobbyArea + c.refugeAreaPerFloor,
                              1,
                            )}
                          </TD>
                        </tr>
                      ...
                      {c.fireCheckLobbyArea > 0 && (
                        <tr>
                          ...
                          <TD>Required (height &gt;24m)</TD>
                          <TD>{fmt(c.fireCheckLobbyArea, 1)}</TD>
                        </tr>
                      )}
                      {c.refugeAreaPerFloor > 0 && (
                        <tr>
                          <TD className="pl-6 text-slate-600">
                            - Refuge area (avg/floor)
                          </TD>
                          <TD>Every 7 floors</TD>
                          <TD>{fmt(c.refugeAreaPerFloor, 1)}</TD>
                        </tr>
                      )}
                      ...
                      <tr className="font-bold bg-slate-200 border-t-2 border-slate-400">
                        ...
                        <TD>
                          {fmt(
                            c.totalCoreAreaPerFloor +
                              c.totalCirculationAreaPerFloor,
                            1,
                          )}
                        </TD>
                      </tr>
              ...
              <p className="mt-1 text-[9px] text-slate-500 italic">
                Core calculations per NBC norms — Method B formula (lift sizing,
                stair count, population-based corridor sizing)
              </p>
            </div>
          </>
        )}
      ...
      <div className="report-page">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Incoming:</strong> 11 kV from grid
              </li>
              <li>
                <strong>Transformer:</strong> 1,000 kVA (on ground floor)
              </li>
              <li>
                <strong>Main LT Panel:</strong> 400V, 3-phase, 1,500A
              </li>
              <li>
                <strong>Per flat:</strong> 10 kW load (40A MCB)
              </li>
              <li>
                <strong>Common areas:</strong> 150 kW
              </li>
              <li>
                <strong>Lifts:</strong> 45 kW (3 × 15 kW)
              </li>
              <li>
                <strong>Water pumps:</strong> 30 kW | <strong>STP:</strong> 15
                kW
              </li>
              <li>
                <strong>Total connected load:</strong> ~900 kW
              </li>
              <li>
                <strong>Demand factor:</strong> 0.7 →{" "}
                <strong>Actual demand:</strong> ~630 kW
              </li>
            </ul>
          </div>
          <div>
            <strong className="block text-slate-800 mb-1 border-b pb-1">
              Backup Power & Earthing
            </strong>
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>DG Set:</strong> 500 kVA (100% common area + 1
                lift/tower)
              </li>
              <li>
                <strong>Location:</strong> Ground floor, exterior
              </li>
              <li>
                <strong>Fuel tank:</strong> 1,500 liters (24-hour backup)
              </li>
              <li>
                <strong>ATS:</strong> Auto start within 10 seconds
              </li>
              <li>
                <strong>Sound enclosure:</strong> &lt;75 dB at 1m
              </li>
              <li>
                <strong>Earthing:</strong> Plate earthing (2 nos.,
                1200×1200×3mm), &lt;1 ohm
              </li>
              <li>
                <strong>Lightning arrestor:</strong> On terrace (ESE type)
              </li>
            </ul>
          </div>
        </div>
        <div className="mb-4 text-[10px]">
          ...
          <ul className="list-disc pl-4 space-y-0.5">
            <li>
              <strong>Floor DB:</strong> 400A capacity
            </li>
            <li>
              <strong>Each flat:</strong> Dedicated 40A SPDB |{" "}
              <strong>Lighting:</strong> 10A circuits | <strong>Power:</strong>{" "}
              16A circuits
            </li>
            <li>
              <strong>AC:</strong> 32A dedicated | <strong>Geyser:</strong> 16A
              dedicated | <strong>Common areas:</strong> Separate 63A panel
            </li>
          </ul>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Source:</strong> Municipal + borewell (backup)
              </li>
              <li>
                <strong>Underground sump:</strong> 65,000 liters (3-day storage,
                5m×4m×3.5m, below Tower 1 basement)
              </li>
              <li>
                <strong>Overhead tanks:</strong> 22,000 liters each (1 per
                tower, SS 304, terrace)
              </li>
              <li>
                <strong>Pumping System:</strong> 2 pumps (duty+standby), 15 HP
                each, 35m head, 100 lpm
              </li>
              <li>
                <strong>Hydro-pneumatic sys:</strong> 500L vessel, 2.5-3.5
                kg/cm², 5 HP pump
              </li>
              <li>
                <strong>Distribution:</strong> 150mm CPVC risers, 25mm flat
                connections, 50mm insulated hot water riser
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Type:</strong> Two-pipe system (Code 11.1)
              </li>
              <li>
                <strong>Horizontal Drainage:</strong> UPVC floor traps, min
                gradient 1:60
              </li>
              <li>
                <strong>Vertical Drainage:</strong> 150mm soil stack, 100mm
                waste stack, 75mm vent stack
              </li>
              <li>
                <strong>Collection:</strong> All drainage to STP (not public
                sewer)
              </li>
              <li>
                <strong>STP capacity:</strong> 25,000 lpd.{" "}
                <strong>Location:</strong> Basement 1
              </li>
              <li>
                <strong>Technology:</strong> Extended aeration + MBR. Zero
      ...
      <div className="report-page">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Type:</strong> Individual split AC units
                (owner-provided)
              </li>
              <li>
                <strong>Outdoor unit space:</strong> 1.0m × 0.6m per bedroom
              </li>
              <li>
                <strong>Location:</strong> Service balcony / external wall
                bracket
              </li>
              <li>
                <strong>Drainage:</strong> Dedicated waste pipe
              </li>
              <li>
                <strong>Electrical:</strong> 32A dedicated point per AC
              </li>
            </ul>
          </div>
          <div>
            <strong className="block text-slate-800 mb-1 border-b pb-1">
              Common Area HVAC & Terrace
            </strong>
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>GF Lobbies:</strong> VRV system (15 TR), 20% fresh air
                intake
              </li>
              <li>
                <strong>Basement:</strong> Mechanical ventilation (15 ACH min).
                10 exhaust fans (5,000 CFM), 8 supply fans (6,000 CFM)
              </li>
              <li>
                <strong>Sensors:</strong> CO sensors with auto-control
              </li>
              <li>
                <strong>Terrace Equipment:</strong> VRV outdoor units, exhaust
                motors, lift machine room ventilation, DG exhaust
              </li>
            </ul>
          </div>
        </div>
        ...
        <table className="w-full border-collapse mb-4 text-[10px]">
          ...
          <tbody>
            <tr className="bg-slate-50 font-semibold text-red-800">
              <TD colSpan={2}>Active Systems</TD>
            </tr>
            <tr>
              <TD className="font-semibold">Wet Riser</TD>
              <TD>
                100mm dia riser. 2 hydrant valves/floor. 25mm × 30m hose reels.
                Jockey (3 HP) + Main pump (20 HP). 50,000L tank.
              </TD>
            </tr>
            <tr>
              <TD className="font-semibold">Sprinkler (Basements)</TD>
              <TD>
                Wet pipe system. 3m × 3m spacing. 68°C fusible link. 15 HP
                dedicated pump.
              </TD>
            </tr>
            <tr>
              <TD className="font-semibold">Fire Extinguishers</TD>
              <TD>
                4 per floor. ABC dry powder (6 kg) & CO₂ (4.5 kg). Quarterly
                inspection.
              </TD>
            </tr>
            <tr>
              <TD className="font-semibold">Fire Alarm</TD>
              <TD>
                Addressable smoke/heat detectors. Call points, hooters, 2-hr
                backup. Security room panel.
              </TD>
            </tr>

            <tr className="bg-slate-50 font-semibold text-orange-800">
              <TD colSpan={2}>Passive Systems</TD>
            </tr>
            <tr>
              <TD className="font-semibold">Passive Defenses</TD>
              <TD>
                1-hour rated doors, 2-hour rated core walls. Fire stopped
                penetrations.
              </TD>
            </tr>
            <tr>
              <TD className="font-semibold">Evacuation</TD>
              <TD>
                Emergency lighting (2-hr backup), photoluminescent exit signage.
              </TD>
            </tr>
          </tbody>
        </table>
      ...
      <div className="report-page">
        ...
        {(() => {
          ...
          const floorplate =
            firstBuilding?.area ||
            Math.round(builtUp / Math.max(1, totalLevels * towers));
          const coreArea =
            buildingCores[0]?.core?.totalCoreAreaPerFloor ||
            Math.round(floorplate * 0.12);
          const netFloorArea = Math.round((floorplate - coreArea) * 0.95);
          const avgUnitCarpet =
            totalUnits > 0 ? Math.round(totalCarpet / totalUnits) : 110;
          ...
          return (
            <>
              <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
                <div>
                  ...
                  <ul className="list-disc pl-4 space-y-0.5 mb-3">
                    <li>
                      <strong>Total Built-up Area:</strong> {fmt(builtUp)} sq.m
                      (FAR: {fmt(achievedFAR, 2)})
                    </li>
                    <li>
                      <strong>Towers:</strong> {towers} Nos. × {maxFloors} upper
                      floors each
                    </li>
                    <li>
                      <strong>Ground Floor Plate:</strong> ~{fmt(floorplate)}{" "}
                      sq.m per tower
                    </li>
                    <li>
                      <strong>Typical Floor Plate:</strong> ~{fmt(floorplate)}{" "}
                      sq.m (excluding core)
                    </li>
                    <li>
                      <strong>Basements:</strong> {maxBasements} levels (B
                      {maxBasements}–B1) for parking
                    </li>
                    <li>
                      <strong>Total Levels:</strong> B{maxBasements} + G +{" "}
                      {maxFloors} = {totalLevels} levels
                    </li>
                  </ul>

                  <SH2 className="!mt-2">
                    Efficiency Calculation (Per Floor)
                  </SH2>
                  <ul className="list-disc pl-4 space-y-0.5">
                    <li>
                      <strong>Floor Plate:</strong> {fmt(floorplate)} sq.m
                    </li>
                    <li>
                      <strong>Deduct Core Area:</strong> {fmt(floorplate)} −{" "}
                      {fmt(coreArea)} = {fmt(floorplate - coreArea)} sq.m
                    </li>
                    <li>
                      <strong>Deduct External Walls (~5%):</strong> −{" "}
                      {fmt(Math.round((floorplate - coreArea) * 0.05))} sq.m
                    </li>
                    <li>
                      <strong>Net Leasable Area/Floor:</strong>{" "}
                      {fmt(netFloorArea)} sq.m
                    </li>
                    <li>
                      <strong>Net per Tower Floor:</strong>{" "}
                      {fmt(Math.round(netFloorArea / towers))} sq.m
                    </li>
                  </ul>
                </div>
                <div>
                      ...
                      {Object.entries(units).length > 0 ? (
                        Object.entries(units).map(([type, count], i) => {
                          const uCarpet = tArea[type]
                            ...
                            : avgUnitCarpet;
                          const uBuiltUp = Math.round(
                            uCarpet / (carpetEff / 100),
                          );
                          ...
                          return (
                            ...
                            >
                              ...
                              <TD>
                                {uCarpet} sq.m carpet / {uBuiltUp} sq.m built-up
                                <br />
                                <span className="text-slate-500">
                                  Balcony: {uBalcony} sq.m
                                </span>
                              </TD>
                            </tr>
                          );
                        ...
                        <>
                          <tr className="bg-slate-50">
                            <TD className="font-semibold">RERA Carpet</TD>
                            <TD>{avgUnitCarpet} sq.m</TD>
                          </tr>
                          <tr>
                            <TD className="font-semibold">Balcony</TD>
                            <TD>{balcony} sq.m</TD>
                          </tr>
                          <tr className="bg-slate-50">
                            <TD className="font-semibold">Built-up</TD>
                            <TD>{avgBuiltUp} sq.m</TD>
                          </tr>
                        </>
                      )}
                    </tbody>
              ...
              <div className="border border-blue-200 bg-blue-50 p-3 rounded mb-4 text-[10px]">
                  ...
                  <tbody>
                    {Object.entries(units).length > 0 ? (
                      ...
                      <tr className="bg-white">
                        ...
                        <TD className="text-center">
                          {fmt((totalUnits || 64) * avgUnitCarpet)}
                        </TD>
                      </tr>
                    )}
                ...
                <div className="grid grid-cols-3 gap-3 mt-2 text-[9px]">
                  <div className="bg-white border border-blue-200 p-2 rounded text-center">
                    <strong className="block text-blue-800 text-[11px]">
                      {totalUnits}
                    </strong>
              ...
              <p className="text-[10px] mb-3 text-slate-600">
                <strong>{totalUnits} units</strong> across{" "}
                <strong>{maxFloors} upper floors</strong> ×{" "}
                <strong>{towers} tower(s)</strong> = approximately{" "}
                <strong>
                  {Math.round(totalUnits / Math.max(1, maxFloors * towers))}{" "}
                  unit(s)/floor/tower
                </strong>
              </p>
            </>
        ...
        <div className="grid grid-cols-2 gap-4 text-[10px]">
          <div>
            <strong className="block text-slate-800 mb-1 border-b pb-1">
              Standard Inclusions (All Units)
            </strong>
            <ul className="list-none space-y-0.5">
              {[
                "Video door phone system",
                "Intercom to security/management",
                "Fire alarm (smoke detectors in corridor)",
                `Provisions for split AC (${Math.max(2, Object.keys(units).length > 0 ? Math.min(4, Object.keys(units).length + 1) : 3)} outdoor unit spaces)`,
                "Provision for geyser (3 nos.)",
                "Provision for chimney (kitchen)",
                "Provision for washing machine (service balcony)",
                "DTH/cable TV points (living + master BR)",
                "Broadband-ready (Cat-6 cabling)",
                "Modular switches (Legrand/Anchor/Havells)",
                "Concealed plumbing & electrical",
                "Quality hardware (Godrej/Dorset locks)",
              ].map((item, i) => (
                <li key={i}>
          ...
          <div>
            ...
            <ul className="list-none space-y-0.5">
              {[
                ...
                "Designer false ceiling (living)",
                "Upgraded tiles (imported)",
                "Premium CP fittings (Grohe/Hansgrohe)",
                "Upgraded kitchen (Hacker/Hafele modular)",
                "Home automation (basic package)",
                "Wallpaper (accent walls)",
                "Additional storage (loft/attic)",
              ].map((item, i) => (
                <li key={i}>
      ...
      <div className="report-page">
        ...
        {(() => {
          ...
          const allocated = split.map((v) =>
            Math.round((v / splitSum) * totalAmenitiesArea),
          );
          ...
          return (
            <>
              <div className="mb-4 text-[10px]">
                ...
                <ul className="list-disc pl-4 space-y-0.5">
                  <li>
                    <strong>Total ground floor area (both towers):</strong>{" "}
                    {towers} × {towerFootprint} sq.m ={" "}
                    {fmt(towers * towerFootprint)} sq.m
                  </li>
                  <li>
                    <strong>Core & circulation:</strong> {towers} × {coreArea}{" "}
                    sq.m = {fmt(towers * coreArea)} sq.m
                  </li>
                  <li>
                    <strong>Available for amenities:</strong>{" "}
                    {fmt(totalAmenitiesArea)} sq.m
                  </li>
                  <li>
                    <strong>Additional open-air amenities:</strong> ~
                    {fmt(metrics?.greenArea?.total || 500)} sq.m (in landscaped
                    areas)
                  </li>
                </ul>
              </div>
              ...
              <table className="w-full border-collapse mb-4 text-[10px]">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i}>
                      <TD className="font-semibold bg-slate-50">
                        {a as string}
                      </TD>
                      <TD>{b as number}</TD>
                      <TD>{c as string}</TD>
        ...
        <table className="w-full border-collapse mb-4 text-[10px]">
          ...
          <tbody>
            {[
              ...
              [
                "Landscaped Gardens",
                `${fmt(metrics?.greenArea?.total || 280)}`,
                "Throughout",
                "Lawns, trees, shrubs",
              ],
            ].map(([a, b, c, d], i) => (
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Boom barrier:</strong> Automatic (RFID/remote)
              </li>
              <li>
                <strong>Security cabin:</strong> 12 sq.m (3m × 4m)
              </li>
              <li>
                <strong>CCTV monitors:</strong> 16-channel DVR
              </li>
              <li>
                <strong>Visitor register:</strong> Digital tablet
              </li>
              <li>
                <strong>Intercom:</strong> Master panel & PA system
              </li>
              <li>
                <strong>Guard:</strong> 24×7 (2 shifts)
              </li>
            </ul>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Pedestrian gate:</strong> RFID card/biometric
              </li>
              <li>
                <strong>Vehicle gate:</strong> RFID tag + boom barrier
              </li>
              <li>
                <strong>Lift access:</strong> RFID card (for residents)
              </li>
              <li>
                <strong>Visitor management:</strong> Digital register +
                photograph
              </li>
              <li>
                <strong>Intercom:</strong> Gate to apartments
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Cameras:</strong> 24 nos. (strategic locations)
              </li>
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Type:</strong> IP cameras (2 MP, day/night)
              </li>
              <li>
                <strong>Recording:</strong> 30-day storage (NVR)
              </li>
              <li>
                <strong>Remote viewing:</strong> Mobile app access
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="report-page">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Requirement:</strong> 1.5 ECS per unit
              </li>
              <li>
                <strong>Total Base:</strong> {totalUnits} × 1.5 ={" "}
                {Math.ceil(totalUnits * 1.5)} ECS minimum
              </li>
              <li>
                <strong>Guest parking (10%):</strong>{" "}
                {Math.ceil(totalUnits * 1.5)} × 0.1 ={" "}
                {Math.ceil(totalUnits * 1.5 * 0.1)} ECS
              </li>
              <li className="font-semibold text-slate-900 border-t pt-1 mt-1">
                <strong>Total required:</strong>{" "}
                {Math.ceil(totalUnits * 1.5) +
                  Math.ceil(totalUnits * 1.5 * 0.1)}{" "}
                ECS
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Available Basement Floor Plate:</strong>{" "}
                {fmt(plot.buildings?.[0]?.area || 800 * towers)} sq.m
              </li>
              <li>
                <strong>Less core & circulation:</strong>{" "}
                {fmt(
                  (plot.buildings?.[0]?.area || 800 * towers) -
                    (buildingCores[0]?.core?.totalCoreAreaPerFloor ||
                      100 * towers),
                )}{" "}
                sq.m usable space
              ...
              <li>
                <strong>Less ramps/turning (~15%):</strong> ~
                {fmt(
                  ((plot.buildings?.[0]?.area || 800 * towers) -
                    (buildingCores[0]?.core?.totalCoreAreaPerFloor ||
                      100 * towers)) *
                ...
                sq.m available for parking
              </li>
            </ul>
            <div className="bg-slate-50 border p-2 rounded mt-1">
              <strong>Est. Capacity/Floor:</strong>{" "}
              {fmt(
                (((plot.buildings?.[0]?.area || 800 * towers) -
                  (buildingCores[0]?.core?.totalCoreAreaPerFloor ||
                    100 * towers)) *
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Requirement (Code 7.1(1)):</strong> 0.5 spaces per unit
              </li>
              <li>
                <strong>Total spaces required:</strong> {totalUnits} × 0.5 ={" "}
                {Math.ceil(totalUnits * 0.5)} spaces
              </li>
              <li>
                <strong>Unit dimension:</strong> 0.8m × 2.5m = 2.0 sq.m
              </li>
              <li>
                <strong>Total area:</strong> {Math.ceil(totalUnits * 0.5)} × 2 ={" "}
                {Math.ceil(totalUnits * 0.5) * 2} sq.m
              </li>
              <li>
                <strong>Location:</strong> Basement Level B1 (near ramp) in
                individual racks
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Location:</strong> Ground level (near entrance)
              </li>
              <li>
                <strong>Capacity:</strong> {Math.ceil(totalUnits * 1.5 * 0.1)}{" "}
                cars
              </li>
              <li>
                <strong>Access:</strong> Separate from resident basement
              </li>
              <li>
                <strong>Monitoring:</strong> CCTV + security
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-[10px]">
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Main circulation aisle:</strong> 6.0m (two-way traffic)
              </li>
              <li>
                <strong>Secondary aisle:</strong> 4.0m (one-way)
              </li>
              <li>
                <strong>Parking bay access:</strong> 3.5m minimum
              </li>
              <li>
                <strong>Parking bay dimensions:</strong> 2.5m (W) × 5.0m (L)
                (Angle: 90°)
              </li>
            </ul>
            <strong className="block text-slate-800 mb-1 border-b pb-1">
            ...
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Direction:</strong> Clockwise/counterclockwise clearly
                marked
              </li>
              <li>
                <strong>Speed limit:</strong> 10 km/hr
              </li>
              <li>
                <strong>Signage:</strong> Directional arrows, level indicators
              </li>
              <li>
                <strong>Lighting:</strong> Well-lit (150 lux minimum)
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-0.5 mb-2">
              <li>
                <strong>Entry Location:</strong> South side (from 6.04m road)
              </li>
              <li>
                <strong>Gate Type:</strong> Boom barrier with RFID (4.0m width)
              </li>
              <li>
                <strong>Exit:</strong> One-way loop within site (clear
                visibility mirrors)
              </li>
            </ul>
            <strong className="block text-slate-800 mb-1 border-b pb-1">
              Pedestrian Circulation (Lifts/Stairs)
            </strong>
            <ul className="list-disc pl-4 space-y-0.5">
              <li>
                <strong>Lifts:</strong> 3 per tower (1.5 m/s, &lt;45s peak wait
                time)
              </li>
              <li>
                <strong>Core Corridors:</strong> 1.5m min width, 2.45m clear
                height
              </li>
              <li>
                <strong>Accessibility:</strong> 1:12 ramps at entry,
                wheelchair-accessible lifts (2.0×1.8m)
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="report-page">
        ...
        {cb ? (
            ...
            return (
              <>
                <div className="grid grid-cols-2 gap-4 text-[9px] mb-4">
                  <div>
                    ...
                    <table className="w-full border-collapse mb-3">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <TD>{fmt(totalCarpet * 0.18)} sqm</TD>
                          <TD>
                            {crore(
                        ...
                        <tr className="bg-slate-50 font-semibold">
                          <TD colSpan={2}>Sub-total (Superstructure)</TD>
                          <TD>
                            {crore(
                    ...
                    <table className="w-full border-collapse mb-3">
                      ...
                      <tbody>
                        <tr>
                          <TD>Basements (Parking)</TD>
                          <TD>{fmt(plotArea * 0.8 * 4)} sqm</TD>
                          <TD>
                            {crore(
                        ...
                        <tr className="bg-slate-50 font-semibold">
                          <TD colSpan={2}>Sub-total (Substructure)</TD>
                          <TD>
                            {crore(
                              (cb.earthwork +
                                cb.structure * 0.2 +
                  ...
                  <div>
                    ...
                    <table className="w-full border-collapse mb-3">
                      ...
                      <tbody>
                        ...
                        <tr className="bg-slate-50 font-semibold">
                          <TD colSpan={2}>Sub-total (Amenities)</TD>
                          <TD>
                            {crore(
                    ...
                    <strong className="block text-slate-800 mb-1 border-b pb-1 pt-2">
                ...
                <div className="grid grid-cols-2 gap-4 text-[10px]">
                  ...
                  <div>
                    <strong className="block text-slate-800 mb-1 border-b pb-1">
                      Sensitivity Analysis (Price Variation)
                    </strong>
                        ...
                        {[-10, 0, 10].map((pct, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TD>
                                {((adjProfit / adjRev) * 100).toFixed(1)}%
                              </TD>
                            </tr>
                          );
                        })}
          ...
          <p className="text-slate-500 italic">
            Financial estimates not loaded. Configure cost parameters in Admin
            Panel.
          </p>
        )}
      </div>
      ...
      <div className="report-page">
        ...
        <p className="text-[9px] text-slate-500 italic mb-3">
          The following figures are computed dynamically by the Keystone AI
          simulation engine based on project parameters configured in the Admin
          Panel. These may differ from the PDF benchmark values above, which use
        ...
        {estimates ? (
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[
                ...
                },
                {
                  ...
                  value: crore(totalRev),
                  sub: "Derived from Market Rates",
                  color: "bg-slate-700",
                },
                {
                ...
                {
                  label: "Project Duration (P10–P90)",
                  value: sim
                    ? `${Math.round(sim.time_p10)} – ${Math.round(sim.time_p90)} mo`
                    : `${fmt(tl?.total_months, 1)} mo`,
                  sub: "Simulation Range",
                  color: "bg-slate-800",
                },
              ].map((k, i) => (
                <div key={i} className={`${k.color} text-white rounded p-3`}>
                  <div className="text-[9px] opacity-75 font-semibold uppercase tracking-wider mb-1">
                    {k.label}
                  </div>
              ...
              <div>
                    ...
                    {[
                      {
                        ...
                        ratio: estimates.cost_breakdown.earthwork / totalCost,
                      },
                      {
                        label: "RCC Structure",
                        ratio: estimates.cost_breakdown.structure / totalCost,
                      },
                      {
                        label: "Finishing Works",
                        ratio: estimates.cost_breakdown.finishing / totalCost,
                      },
                      {
                        label: "MEP Services",
                ...
                {estimates.efficiency_metrics && (
                  <div className="mt-3 p-2 rounded border border-slate-200 bg-slate-50 text-[9px]">
                    <strong>Model Efficiency:</strong> Achieved{" "}
                    {estimates.efficiency_metrics.achieved.toFixed(1)}% vs
                    Target {estimates.efficiency_metrics.target.toFixed(1)}%
              ...
              <div>
                ...
                <table className="w-full border-collapse text-[9px]">
                  <thead>
                    <tr>
                      <TH>Phase</TH>
                      <TH className="text-right">Duration Range</TH>
                      <TH className="text-right">% of Total</TH>
                    </tr>
                  </thead>
                  <tbody>
                    {tl &&
                      [
                        {
                          label: "Excavation",
                          ratio: tl.phases.excavation / tl.total_months,
                        },
                        {
                          label: "Foundation",
                          ratio: tl.phases.foundation / tl.total_months,
                        },
                        {
                          label: "Structural Build",
                          ratio: tl.phases.structure / tl.total_months,
                        },
                        {
                          label: "Finishing",
                          ratio: tl.phases.finishing / tl.total_months,
                        },
                        ...(tl.phases.contingency
                          ? [
                              {
                                label: "Contingency Buffer",
                                ratio: tl.phases.contingency / tl.total_months,
                              },
                            ]
                          : []),
                      ].map((row, i) => {
                        const p10 = sim
                          ? sim.time_p10 * row.ratio
                          : tl.total_months * row.ratio * 0.95;
                        const p90 = sim
                          ? sim.time_p90 * row.ratio
                          : tl.total_months * row.ratio * 1.05;
                        return (
                          <tr
                            key={i}
                            className={i % 2 === 0 ? "bg-slate-50" : ""}
                  ...
                  <tbody>
                    {estimates.breakdown.map((b, i) => (
                      <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                        ...
                        <TD className="text-right">
                          {b.cost.ratePerSqm ? fmt(b.cost.ratePerSqm) : "—"}
                        </TD>
                        <TD className="text-right font-semibold">
                          {crore(b.cost.total)}
                        </TD>
                        <TD className="text-right">
                          {b.utilityCost ? crore(b.utilityCost) : "—"}
                        </TD>
                        <TD className="text-right">
                          {fmt(b.timeline.total, 1)} mo
                        </TD>
                        <TD className="text-right">
                          {fmt(b.timeline.structure, 1)} mo
                        </TD>
                        <TD className="text-right">
                          {fmt(b.timeline.finishing, 1)} mo
                        </TD>
                      </tr>
                    ))}
                    <tr className="bg-slate-800 text-white font-bold">
                      <TD colSpan={4} className="font-bold text-white">
                        Total (All Towers)
                      </TD>
                      <TD className="font-bold text-white text-right">
                        {crore(
          ...
          <p className="text-slate-500 italic">
            Simulation data not available. Run the feasibility simulator to
            generate dynamic cost estimates.
          </p>
        )}
      </div>
      <PageBreak />
      ...
      <div className="report-page">
        ...
        {(() => {
          const m10 = sim
            ? Math.round(sim.time_p10)
            : (tl?.total_months || 30) * 0.95;
          const m90 = sim
            ? Math.round(sim.time_p90)
            : (tl?.total_months || 30) * 1.05;

          const scale10 = (m: number) =>
            Math.max(1, Math.round(m * (m10 / 30)));
          const scale90 = (m: number) =>
            Math.max(1, Math.round(m * (m90 / 30)));

          const range = (startM: number, endM: number) => {
            const s10 = scale10(startM);
            const s90 = scale90(startM);
            const e10 = scale10(endM);
            const e90 = scale90(endM);
          ...
          const methodology = [
            {
              title: `Phase 1: Preparation (${range(1, 2)})`,
              items: [
                "Site mobilization & temp facilities",
                "Boundary hoarding (3m, branded)",
                "Site office, labor colony",
                "Material storage yard",
                "Soil investigation (2 boreholes)",
                "Survey & setting out",
              ],
            },
            {
              title: `Phase 2: Foundation (${range(2, 5)})`,
              items: [
                "Basement excavation (~10k cu.m)",
                "Raft foundation (500mm)",
                "Pile drilling (if req) & pile cap",
                "Basement retaining walls (300mm)",
                "Ext. waterproofing & termite check",
                "Backfilling",
              ],
            },
            {
              title: `Phase 3: Basements (${range(5, 9)})`,
              items: [
                "Basement-4 to Basement-1",
                "Column & wall formwork",
                "Reinforcement & M30 Concreting",
                "Basement slabs (200mm)",
                "Curing (7-14 days)",
                "Services sleeves & Conduits",
                "Formwork striking & reuse",
              ],
            },
            {
              title: `Phase 4: Structure (${range(9, 20)})`,
              items: [
                "Ground to Floor 8 (parallel)",
                "Column casting (floor-by-floor)",
                "Beam & slab formwork",
                "Reinforcement & concreting",
                "Cycle: 10-12 days per floor",
                "Core shear walls (continuous)",
                "Lift shaft & Staircases",
              ],
            },
            {
              title: `Phase 5: Ext. Masonry (${range(15, 22)})`,
              items: [
                "External walls (230mm AAC)",
                "Internal partitions (115mm AAC)",
                "Plastering (external & internal)",
                "Window/door frames",
              ],
            },
            {
              title: `Phase 6: Roof/Terrace (${range(20, 21)})`,
              items: [
                "Terrace slab concreting",
                "Mumty/lift room construction",
                "Water tanks (2 × 22k L)",
                "Solar panel mounting",
                "Terrace waterproofing (APP)",
                "Parapet construction",
              ],
            },
            {
              title: `Phase 7: MEP Rough (${range(18, 23)})`,
              items: [
                "Electrical conduit laying",
                "Plumbing pipes (concealed)",
                "HVAC duct installation",
                "Fire-fighting pipes",
                "Service shaft completion",
              ],
            },
            {
              title: `Phase 8: Ext. Finish (${range(21, 24)})`,
              items: [
                "External plastering",
                "Facade paint/texture",
                "Windows & grills installation",
                "External lighting",
                "Signage",
              ],
            },
            {
              title: `Phase 9: Int. Finish (${range(22, 26)})`,
              items: [
                "Internal plastering/POP",
                "Flooring (tiles/vitrified)",
                "Wall tiling (baths/kitchen)",
                "Painting (2-3 coats)",
                "False ceiling (if req)",
            ...
            {
              title: `Phase 10: MEP Fit-out (${range(23, 27)})`,
              items: [
                "Lift install & commissioning",
                "DG set & Transformer",
                "Panels & Fire alarm",
                "CCTV & access control",
                "STP & Solar commissioning",
                "Water pumps & testing",
              ],
            },
            {
              title: `Phase 11: Ext. Works (${range(24, 28)})`,
              items: [
                "Landscaping (soft & hard)",
                "Swimming pool construction",
                "Pavements & driveways",
                "Boundary wall & gate",
                "External lighting & branding",
              ],
            },
            {
              title: `Phase 12: Handover (${range(27, 30)})`,
              items: [
                "Snag list clearance",
                "Deep cleaning",
                "Amenity equipment",
                "Furniture (common areas)",
                "Trial runs (all systems)",
                "Occupation Certificate",
                "Unit handover (phased)",
              ],
            },
          ];
          ...
          return (
            <div className="grid grid-cols-2 gap-4 text-[8px] mb-4">
              <div>
                <SH2 className="!mt-0">
                  13.1 Construction Sequence (Phases 1-6)
                </SH2>
                <div className="grid grid-cols-2 gap-2">
                  {methodology.slice(0, 6).map((ph, i) => (
                    <div key={i} className="mb-1">
                      <strong className="block text-slate-800 mb-0.5 border-b border-slate-200 pb-[1px]">
                        {ph.title}
                      </strong>
        ...
        {tl ? (
          <>
            <table className="w-full border-collapse mb-4 text-[10px]">
              <thead>
                <tr>
                  <TH>Major Phase</TH>
                  <TH>Estimated Duration Range</TH>
                  <TH>Critical Path</TH>
                </tr>
              </thead>
              <tbody>
                {[
                  {
                    label: "Approvals & Mobilization (Phase 1)",
                    ratio: Math.min(tl.phases.excavation, 3) / tl.total_months,
                    cp: "Yes",
                  },
                  {
                    label: "Excavation & Basements (Phases 2-3)",
                    ratio: tl.phases.foundation / tl.total_months,
                    cp: "Yes",
                  },
                  {
                    label: "Superstructure (Phase 4)",
                    ratio: tl.phases.structure / tl.total_months,
                    cp: "Yes",
                  },
                  {
                    label: "Masonry & Rough-in (Phases 5-7)",
                    ratio: (tl.phases.finishing * 0.4) / tl.total_months,
                    cp: "No (Parallel)",
                  },
                  {
                    label: "Finishing & MEP Fit-out (Phases 8-10)",
                    ratio: (tl.phases.finishing * 0.6) / tl.total_months,
                    cp: "Yes",
                  },
                  {
                    label: "External Works & Handover (Phases 11-12)",
                    ratio: (tl.phases.contingency || 0) / tl.total_months,
                    cp: "Yes",
                  },
                ].map((row, i) => {
                  const p10 = sim
          ...
          <p className="text-slate-500 italic text-sm">
            Timeline parameters not available. Configure in Admin Panel.
          </p>
        )}
      </div>
      <PageBreak />
      ...
      <div className="report-page">
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px]">
          <div>
            ...
            <p className="text-[8px] italic text-slate-500 mb-1">
              Peak Manpower (Month 15-20)
            </p>
            <table className="w-full border-collapse mb-4">
              <thead>
                <tr>
            ...
            <p className="text-[9px] text-slate-600 mt-1">
              * Average Manpower: 200-250 (over{" "}
              {tl?.total_months ? Math.round(tl.total_months) : 30} months)
            </p>
          </div>
          <div>
        ...
        {(() => {
          ...
          const phases = [
            {
              label: "1. Approvals & Mobilization",
              start: 0,
              end: 2,
              color: "#64748b",
            },
            {
              label: "2. Excavation & Foundation",
              start: 1,
              end: 5,
              color: "#dc2626",
            },
            {
              label: "3. Basement Construction",
              start: 4,
              end: 9,
              color: "#ea580c",
            },
            {
              label: "4. Superstructure (G+8)",
              start: 8,
              end: 20,
              color: "#0369a1",
            },
            {
              label: "5. External Masonry",
              start: 14,
              end: 22,
              color: "#7c3aed",
            },
            {
              label: "6. Roof & Terrace",
              start: 19,
              end: 21,
              color: "#475569",
            },
            ...
            {
              label: "8. External Finishing",
              start: 21,
              end: 24,
              color: "#16a34a",
            },
            {
              label: "9. Internal Finishing",
              start: 22,
              end: 26,
              color: "#15803d",
            },
            {
              label: "10. MEP Final Fit-out",
              start: 23,
              end: 27,
              color: "#1d4ed8",
            },
            {
              label: "11. External Works",
              start: 24,
              end: 28,
              color: "#b45309",
            },
            {
              label: "12. Handover & OC",
              start: 27,
              end: 30,
              color: "#4f46e5",
            },
        ...
        <p className="text-[8px] mt-1 text-slate-500 italic">
          Critical Path: Approvals → Foundation → Basements → Superstructure →
          MEP → Finishing → OC
        </p>
      </div>
      <PageBreak />
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px] mb-4">
          <div>
            {(() => {
              ...
              const eqCost = uw?.promoterEquity || totalCapReq * 0.4;
                      ...
                      {cb
                        ? [
                            ...
                            [
                              "Construction",
                              constructionCost,
                              (constructionCost / totalCapReq) * 100,
                            ],
                            [
                              "Soft Costs (~15%)",
                              softCosts,
                              (softCosts / totalCapReq) * 100,
                            ],
                            [
                              "Contingency (2%)",
                              contingency,
                              (contingency / totalCapReq) * 100,
                            ],
                          ].map(([c, a, p], i) => (
                            <tr key={i}>
                              <TD className="font-semibold bg-slate-50">
          ...
          <div>
            {(() => {
              const landCost = actualLandCost;
              const totalCapReq =
                landCost + totalCost + totalCost * 0.15 + totalCost * 0.02;
              const eqCost = uw?.promoterEquity || totalCapReq * 0.4;
              const debtCost = uw?.requestedLoanAmount || totalCapReq * 0.46;
              const advCost = Math.max(0, totalCapReq - eqCost - debtCost);
              const interestRateText = uw?.targetInterestRate
                ? `${uw.targetInterestRate}% p.a.`
                : "12% p.a.";
            ...
            <div className="mt-2 text-[8px] text-slate-600 space-y-0.5">
              <p>
                <strong>Debt Terms:</strong>
              </p>
              <p>• Loan-to-Value (LTV): 60% of project cost</p>
              <p>
                • Tenure: {uw?.loanTenureMonths || 36} months (incl. 6-month
                moratorium)
              </p>
                ...
                <tbody>
                  {[
                    ...
                    [
                      ...
                      "19%",
                      revCr - p16 - p23 * 2 - p19,
                      (revCr - p16 - p23 * 2 - p19) * 0.8,
                    ],
                  ].map(([q, u, p, sv, col], i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                      <TD className="font-semibold">{q as string}</TD>
                      <TD>{u as number}</TD>
                      <TD>{p as string}</TD>
                      <TD className="text-right">
                        {(sv as number).toFixed(2)}
                      </TD>
                      <TD className="text-right">
                        {(col as number).toFixed(2)}
                      </TD>
                    </tr>
                  ))}
                  <tr className="bg-slate-800 text-white font-bold">
                    <TD className="font-bold text-white">Total</TD>
                    <TD className="font-bold text-white">{units}</TD>
                    <TD className="font-bold text-white">100%</TD>
                    <TD className="font-bold text-white text-right">
                      {revCr.toFixed(2)}
                    </TD>
                    <TD className="font-bold text-white text-right">
              ...
              <div className="grid grid-cols-2 gap-4">
                <table className="w-full border-collapse text-[9px]">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <TD>Total Sales</TD>
                      <TD className="text-right">{revCr.toFixed(2)}</TD>
                    </tr>
                    <tr>
                      <TD>Less: GST (input credit)</TD>
                    ...
                    <tr className="bg-slate-50 font-semibold">
                      <TD colSpan={2}>Less: Project Cost</TD>
                    </tr>
                    <tr>
                      <TD>Land</TD>
                      <TD className="text-right">
                    ...
                    <tr className="font-bold text-green-700">
                      <TD>Gross Profit</TD>
                      <TD className="text-right">
                        {(revCr - totalCapReq / 10000000).toFixed(2)}
                      </TD>
                    </tr>
                    <tr>
                      <TD>Gross Margin</TD>
                      <TD className="text-right font-semibold">
                        {(
                          ((revCr - totalCapReq / 10000000) / revCr) *
                          100
                        ).toFixed(1)}
                        %
                      </TD>
                ...
                <table className="w-full border-collapse text-[9px]">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <TD>Brokerage (2%)</TD>
                      <TD className="text-right">
                        {(revCr * 0.02).toFixed(2)}
                      </TD>
                    </tr>
                    <tr className="font-semibold">
                      <TD>Total OpEx</TD>
                      <TD className="text-right">
                        {(revCr * 0.05).toFixed(2)}
                      </TD>
                    </tr>

                    {(() => {
                      const ebitda =
                        revCr - totalCapReq / 10000000 - revCr * 0.05;
                      ...
                      return (
                        <>
                          <tr className="font-bold text-blue-700 bg-blue-50">
                            <TD>EBITDA</TD>
                            <TD className="text-right">{ebitda.toFixed(2)}</TD>
                          </tr>
                          <tr>
                            <TD>EBITDA Margin</TD>
                            <TD className="text-right text-blue-700 font-semibold">
                              {((ebitda / revCr) * 100).toFixed(1)}%
                            </TD>
                          </tr>
                          <tr>
                            <TD>Less: Interest on Debt</TD>
                            <TD className="text-right">
                              {(interestExp / 10000000).toFixed(2)}
                            </TD>
                          </tr>
                          <tr className="font-semibold">
                            <TD>PBT</TD>
                            <TD className="text-right">{pbt.toFixed(2)}</TD>
                          </tr>
                          <tr>
                            <TD>Less: Income Tax (25%)</TD>
                            <TD className="text-right">{tax.toFixed(2)}</TD>
                          </tr>
                          <tr className="font-bold text-green-700 bg-green-50">
                            <TD>PAT</TD>
                            <TD className="text-right">{pat.toFixed(2)}</TD>
                          </tr>
                          <tr>
                            <TD>PAT Margin</TD>
                            <TD className="text-right font-semibold">
                              {((pat / revCr) * 100).toFixed(1)}%
                            </TD>
                          </tr>
                        </>
                      );
      ...
      <div className="report-page">
        ...
        <table className="w-full border-collapse mb-4 text-[9px]">
          ...
          <tbody>
            {(() => {
              const baseRevCr =
                (totalRev || totalCarpet * 11000 * 10.764) / 10000000;
              const totalCostFullCr =
                (totalCost * 1.17 + actualLandCost) / 10000000;
              const debtCostTraced =
                uw?.requestedLoanAmount || totalCostFullCr * 10000000 * 0.46;
              const interestExpCr =
                (debtCostTraced * ((uw?.targetInterestRate || 12) / 100)) /
                10000000;
              const equityCr =
                (uw?.promoterEquity || totalCostFullCr * 10000000 * 0.4) /
                10000000;
                ...
                return (
                  ...
                  >
                    <TD className="font-semibold">
                      {pct === 0 ? "Base (0%)" : `${pct > 0 ? "+" : ""}${pct}%`}
                    </TD>
                    <TD className="text-right">{rev.toFixed(2)}</TD>
                    <TD
                      className={`text-right ${pat < 0 ? "text-red-600" : "text-green-700"} font-semibold`}
                    >
                      {pat.toFixed(2)}
                    </TD>
            ...
            {[
              ...
              return (
                ...
                >
                  ...
                  <TD
                    className={`text-right ${patNum < 7 ? "text-red-600" : "text-green-700"} font-semibold`}
                  >
                    {pat}
                  </TD>
        ...
        <p className="text-[8px] text-slate-500 italic mb-4">
          Break-even on Cost: +19% overrun (reduces PAT to zero)
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px]">
          <div>
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Authority: Director, Town & Country Planning / Municipal Corp
            </p>
            ...
            <p className="mt-1 text-[8px]">
              ...
              <strong>Validity:</strong> 5 years (≥15m buildings)
            </p>
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Authority: Director, Fire Services,{" "}
              {plot.regulation?.location?.replace(" Building Code", "") ||
                "State"}
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
            ...
            <p className="mt-1 text-[8px]">
              ...
              <strong>Validity:</strong> Till OC stage
            </p>
          </div>
          <div>
            <strong className="block text-slate-700 mb-1 border-b pb-1">
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Authority: Self-certification (Category A building)
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
            ...
            <p className="mt-1 text-[8px] mb-2">
              <strong>Timeline:</strong> At OC stage &nbsp;|&nbsp;{" "}
              <strong>Validity:</strong> N/A (self-cert)
            </p>

            <strong className="block text-slate-700 mb-1 border-b pb-1">
              Utility Connections (Temp)
            </strong>
            <p className="text-slate-500 text-[8px] mb-1">
      ...
      <div className="report-page">
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px] mb-6">
          <div>
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Frequency: Quarterly | Inspector: Empanelled TPIA
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Authority: Self-certification by Architect
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
            ...
            <p className="mt-1 text-[8px]">
              <strong>Timeline:</strong> Within 7 days / Deemed acceptance if no
              comments
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px]">
          <div>
            ...
            <p className="text-slate-500 text-[8px] mb-1">
              Authority: Competent Authority (Town Planning/MC)
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
            ...
            <p className="mt-1 text-[8px] mb-2">
              <strong>Timeline:</strong> 18 days &nbsp;|&nbsp;{" "}
              <strong>Deemed:</strong> 60 days
            </p>

            <strong className="block text-slate-700 mb-1 border-b pb-1">
              Completion Certificate (RERA)
            </strong>
            <p className="text-slate-500 text-[8px] mb-1">
              Timeline: Within 30 days of Occupation Certificate
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
          ...
          <div>
            {(() => {
              ...
              return (
                <>
                  ...
                  <p className="text-slate-500 text-[8px] mb-1">
                    Electricity ({fmt(elecLoad)} kW load) — UHBVN/DHBVN
                  </p>
                  <ul className="list-disc pl-3 space-y-[1px] text-slate-600 mb-2">
                    <li>OC, Load calc, CEI approval, Electrical cert</li>
                    <li>
                      Timeline: 45-60 days | Cost: {lakh(elecLoad * 2000)} -{" "}
                      {lakh(elecLoad * 2500)}
                    </li>
                  </ul>
                  <p className="text-slate-500 text-[8px] mb-1">
                    Water ({fmt(waterDemand)} L/day) — Municipal Corp / HUDA
                  </p>
                  <ul className="list-disc pl-3 space-y-[1px] text-slate-600 mb-2">
                    <li>OC + Plumbing completion certificate</li>
                    <li>
                      Timeline: 30-45 days | Cost: {lakh(waterDemand * 18)} -{" "}
                      {lakh(waterDemand * 25)}
                    </li>
                  </ul>
                  <p className="text-slate-500 text-[8px] mb-1">
                    Sewerage ({fmt(sewGen)} L/day) — Municipal Corp
                  </p>
                  <ul className="list-disc pl-3 space-y-[1px] text-slate-600 mb-2">
                    <li>OC, STP completion cert, Zero-discharge compliance</li>
                    <li>
                      Timeline: 30 days | Cost: {lakh(sewGen * 12)} -{" "}
                      {lakh(sewGen * 18)}
                    </li>
                  </ul>
                </>
            ...
            <strong className="block text-slate-700 mb-1 border-b pb-1">
              Conveyance Deed (to Society/RWA)
            </strong>
            <p className="text-slate-500 text-[8px] mb-1">
              Timeline: Within 3 months of formation of RWA
            </p>
            <ul className="list-disc pl-3 space-y-[1px] text-slate-600">
            ...
            <p className="mt-1 text-[8px]">
              <strong>Registration:</strong> At Sub-Registrar office
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="report-page">
        ...
        <SH2 className="mt-2 text-blue-800">
          17.1 Project Risks & Mitigation
        </SH2>
        {[
          {
            cat: "Construction Risks",
            rows: [
              [
                "Delay in approvals",
                "Medium",
                "High",
                [
                  "Early application (1 month lead)",
              ...
              [
                ...
                [
                  ...
                  "Contingency in foundation design",
                  "Experienced geotech engineer",
                ],
              ],
              [
                "Labor shortage",
                ...
                [
                  ...
                  "Skilled labor retention (incentives)",
              ...
              [
                ...
                "Medium",
                [
                  "Bulk procurement (advance orders)",
                  "Price escalation clause with vendors",
              ...
              [
                ...
                "Low",
                [
                  "Monsoon planning (waterproofing priority)",
                  "Protective measures (tarpaulin)",
                  "Float in schedule (buffer)",
                ],
              ],
              [
                "Quality issues",
          ...
          },
          {
            ...
            rows: [
              ...
              [
                ...
                "Medium",
                [
                  "Prepayment from sales",
                  "Fixed-rate loan (if possible)",
            ...
            rows: [
              [
                ...
                [
                  "Phased launches",
                  "Price flexibility",
                  "Product differentiation (amenities)",
              ...
              [
                ...
                "Medium",
                [
                  "Unique selling points (location, design)",
                  "Better amenities",
                  "Brand building",
                ],
              ],
              [
                ...
                "Low",
                "Medium",
                ["Compliance buffer", "Flexibility in design", "Legal advice"],
              ],
            ],
          },
          {
            cat: "Operational Risks",
            rows: [
              [
                "Contractor default",
                "Low",
      ...
      <div className="report-page">
        ...
        {(() => {
          ...
          const greenPass =
            greenItems?.filter((g: any) => g.status === "pass").length ?? 0;
          ...
          const recommendation =
            pct >= 80
              ? "APPROVE — Standard Conditions"
              : pct >= 65
                ? "CONDITIONAL APPROVAL"
                : "REFER — Additional Due Diligence Required";
          const bannerCls =
            pct >= 80
          ...
          const criteria = [
            {
              ...
              score: promoterScore,
              max: 20,
              detail: `${towers} towers, ${totalUnits} units, ${carpetEff.toFixed(0)}% carpet eff.`,
            },
            {
              label: "Project Viability",
              score: viabilityScore,
              max: 30,
              detail: `ROI ${roiPct.toFixed(1)}%, FAR ${farCompliant ? "✔" : "✗"}, GC ${gcCompliant ? "✔" : "✗"}, Parking ${parkCompliant ? "✔" : "✗"}`,
            },
            {
              label: "Financial Structuring",
              score: finScore,
              max: 25,
              detail: `DSCR ${dscr.toFixed(2)}, Profit ${crore(profit)}`,
            },
            {
              label: "Location & Marketability",
              score: locationScore,
              max: 20,
              detail: `${greenPass}/${greenCount} green items, demand indicators positive`,
            },
            {
              ...
              max: 5,
              detail:
                farCompliant && gcCompliant && parkCompliant
                  ? "All key regulations met"
                  : "Minor non-conformances noted",
            },
          ];
          ...
          return (
            <>
              <table className="w-full border-collapse mb-4 text-[10px] mt-2">
                ...
                <tbody>
                  {criteria.map(({ label, score, max, detail }, i) => {
                    ...
                    return (
                      <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                        ...
                        <TD
                          className={`text-center font-bold text-[11px] ${g.cls}`}
                        >
                          {g.label}
                        </TD>
                  ...
                  <tr className="bg-slate-800 text-white font-bold border-t-2 border-slate-600">
                    ...
                    <TD className="text-center text-white font-bold text-[13px]">
                      {totalScore}
                    </TD>
                    <TD className="text-center text-slate-300">{totalMax}</TD>
                    <TD
                      className={`text-center font-bold text-[13px] ${overallGrade.cls}`}
                    >
                      {overallGrade.label}
                    </TD>
                    <TD className="text-slate-300 text-[8px]">
                      {pct.toFixed(0)}% — {riskLabel}
                    </TD>
                  </tr>
                </tbody>
              </table>
              ...
              <div
                className={`border p-3 rounded text-center mb-1 ${bannerCls}`}
              >
                <strong className="text-base">
                  Recommended Rating: {overallGrade.label} ({riskLabel})
                </strong>
      ...
      <div className="report-page">
        ...
        <table className="w-full border-collapse mb-2 text-[9px]">
          <thead>
            <tr>
              <TH className="text-left w-1/4">Aspect</TH>
              <TH>Requirement</TH>
              <TH>Proposed</TH>
              <TH>Status</TH>
            </tr>
            ...
            {[
              ...
              },
              {
                ...
                ok:
                  !plot.regulation?.geometry?.max_building_height?.value ||
                  maxHeight <=
                    plot.regulation.geometry.max_building_height.value + 0.5,
              },
              {
                asp: "Setback — Front",
              ...
              <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                ...
                <TD
                  className={`font-bold ${ok ? "text-green-700" : "text-red-600"}`}
                >
        ...
        <div className="grid grid-cols-2 gap-4 text-[9px]">
          <div>
            ...
            <p className="text-[8px] text-blue-700 font-semibold mt-1">
              Technical Score: 9/10 (pending soil investigation)
            </p>
            ...
            <p className="text-[8px] text-green-700 font-semibold mt-1">
              ✔ Adequate quality assurance protocols in place
            </p>
          </div>
          <div>
              ...
              <tbody>
                {[
                  ...
                  [
                    "Location",
                    typeof plot.location === "object"
                      ? String(plot.regulation?.location || "Defined in model")
                      : String(plot.location || "Defined in model"),
                  ],
                  [
                    "Zoning",
                    `${project.intendedUse || "Residential"} (Group Housing)`,
                  ],
                  ["Towers", `${towers} Nos.`],
                  [
                    "Structure",
                  ...
                  [
                    "Avg Unit Carpet",
                    `~${totalUnits > 0 ? Math.round(totalCarpet / totalUnits) : "—"} sq.m`,
                  ],
                  [
                    "Parking Provided",
                    `${parkProv} ECS (${parkProv >= parkReq ? `${parkReq > 0 ? Math.round(((parkProv - parkReq) / parkReq) * 100) : 0}% surplus` : "deficit"})`,
                  ],
                ].map(([p, v], i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
      ...
      <div className="report-page">
        <div className="mt-12 p-6 bg-slate-800 text-white rounded text-center">
          ...
          <p className="text-sm text-slate-300 max-w-lg mx-auto">
            This report was generated dynamically by the Keystone Automated
            Feasibility Engine. All standard specifications are based on{" "}
            {plot.regulation?.location
              ? regLabel(plot.regulation?.location, true)
              : "National Building Code 2016"}
            . Project-specific data is derived from the active design model.
            Verify all metrics with local municipal regulations before financial
            commitment.
          </p>
        </div>
      </div>
