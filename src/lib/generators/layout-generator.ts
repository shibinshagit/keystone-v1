...
interface LayoutParams {
  ...
  /** When true, road-facing ground floor units (one strip side) are removed.
   *  Used to reserve the road-facing ground-floor strip for non-residential uses. */
  groundFloorRoadSideReduction?: boolean;
  /** Center of the parent plot, used to determine which side of the building is facing outward (peripheral) */
  plotCentroid?: [number, number];
}
...
function getColorForUnitType(unitName: string): string {
  ...
  if (name.includes("retail") || name.includes("shop")) return "#D94D8B"; // Pink/Magenta (matches Retail hue 340)
  if (name.includes("office")) return "#5B8FB9"; // Steel Blue (matches Office hue 200)
...
function createRotatedRect(
  ...
  const corners = [
    {
      dist: Math.sqrt(wHalf ** 2 + hHalf ** 2),
      angle: bearing + (Math.atan2(-wHalf, hHalf) * 180) / Math.PI,
    }, // NW
    {
      dist: Math.sqrt(wHalf ** 2 + hHalf ** 2),
      angle: bearing + (Math.atan2(wHalf, hHalf) * 180) / Math.PI,
  ...
  const ring = corners.map(
    (c) =>
      turf.destination(center, c.dist, c.angle, { units: "meters" }).geometry
        .coordinates,
  );
  ring.push(ring[0]);
...
export function generateBuildingLayout(
  ...
  if (rotationAngle !== 0) {
    workingPoly = turf.transformRotate(buildingPoly, -rotationAngle, {
      pivot: center,
    });
  }
  ...
  // Branch early to dedicated layout generators.
  if (intendedUse === BuildingIntendedUse.Retail || intendedUse === 'Retail') {
    return generateRetailLayout(buildingPoly, workingPoly, params, rotationAngle, center);
  }
  ...
  if (intendedUse === "Residential") {
    ...
    intendedUse === "Institutional" ||
    intendedUse === "Public" ||
    intendedUse === "Hospitality"
  ) {
  ...
  const liftArea =
    passengerLifts * 2.5 + serviceLifts * 5.0 + stretcherLifts * 6.0;
  ...
  if (
    popPerFloor > 500 &&
    (intendedUse === "Commercial" || intendedUse === "Institutional")
  ) {
    numStairs = 3;
  }
  ...
  if (intendedUse === "Commercial")
    shaftArea += Math.ceil(floorArea / 1000) * 3;
  else if (intendedUse === "Institutional") shaftArea += numPhysicalCores * 4.5;

  // Garbage/Others
  if (intendedUse === "Residential" && assumedUnitsPerFloor >= 6)
    shaftArea += 0.7;
  else if (intendedUse === "Hospitality" || intendedUse === "Institutional")
    shaftArea += 1.5;
  ...
  const calculatedCoreAreaPerFloor =
    liftArea + stairArea + lobbyArea + corridorArea + shaftArea + fireLobbyArea;
  ...
  if (intendedUse === "Residential") {
    ...
    healthyMin = 0.3;
    healthyMax = 0.4;
  } else if (intendedUse === "MixedUse" || params.subtype === "mixed") {
    healthyMin = 0.25;
    healthyMax = 0.35;
  }
  ...
  if (coreRatio < healthyMin || coreRatio > healthyMax) {
    console.warn(
      `[Audit] Core ratio ${(coreRatio * 100).toFixed(1)}% is outside healthy range (${healthyMin * 100}-${healthyMax * 100}%) for ${intendedUse}!`,
    );
  } else {
    console.log(
      `[Audit] Core ratio ${(coreRatio * 100).toFixed(1)}% is within healthy bounds (${healthyMin * 100}-${healthyMax * 100}%) for ${intendedUse}.`,
    );
  }
  ...
  if (coreAreaPerNode < minCoreArea) {
    console.warn(
      `[Layout Gen] Core calculation (${coreAreaPerNode.toFixed(1)} sqm) is below healthy minimum (${minCoreArea.toFixed(1)} sqm). Enforcing minimum benchmark area.`,
    );
  ...
  const depth = turf.distance([bbox[0], bbox[1]], [bbox[0], bbox[3]], {
    units: "meters",
  });
  ...
  console.log(
    `[Layout Gen] Computed Core -> Pop: ${popPerFloor.toFixed(0)}, Lifts: ${numLifts}, Stairs: ${numStairs}, Area: ${finalCoreArea.toFixed(1)} sqm, Dims: ${coreW.toFixed(1)}x${coreD.toFixed(1)}`,
  );

  const createCoreAtPoint = (
    point: Feature<any>,
    id: string,
    ...
    const s = turf.destination(pt, coreD / 2 / 1000, 180).geometry
      .coordinates[1];
    const e = turf.destination(pt, coreW / 2 / 1000, 90).geometry
      .coordinates[0];
    const w = turf.destination(pt, coreW / 2 / 1000, -90).geometry
      .coordinates[0];
  ...
  console.log(
    "[Layout Generator] Generating cores for subtype:",
    params.subtype,
  );

  if (params.subtype === "lshaped") {
    console.log(
      "[Layout Generator] L-Shape detected - placing core at junction",
    );
    ...
    console.log(
      "[Layout Generator] T-Shape detected - placing core at junction",
    );
    const [minX, minY, maxX, maxY] = bbox;
    ...
    const candidates = [
      ...
      turf.point([midX, maxY - (maxY - minY) * 0.4]),
      turf.point([midX, maxY - (maxY - minY) * 0.3]),
      turf.point([midX, minY + (maxY - minY) * 0.5]), // Center fallback
    ];
    ...
    const leftCandidates = [
      ...
      turf.point([minX + (maxX - minX) * 0.2, midY]),
      turf.point([minX + (maxX - minX) * 0.3, midY]),
    ];

    const rightCandidates = [
      turf.point([maxX - (maxX - minX) * 0.25, midY]),
      turf.point([maxX - (maxX - minX) * 0.2, midY]),
      turf.point([maxX - (maxX - minX) * 0.3, midY]),
    ];
    ...
    console.log(
      "[Layout Generator] Default core placement (subtype:",
      params.subtype,
      ")",
    );
  ...
  if (cores.length === 0) {
    ...
    if (core)
      cores.push({ id: "core-fallback", type: "Lobby", geometry: core });
  }
  ...
  if (shouldInclude("Electrical") && !params.exactTypologyAllocation) {
    try {
      ...
      if (isHorizontal) {
        ...
        coreEdgeCenter = turf.point([
          coreBox[2],
          (coreBox[1] + coreBox[3]) / 2,
        ]);
        shiftDirection = 90;
        offsetDist = elecW / 2;
      } else {
      ...
      const elecWDeg =
        elecW / ((40008000 / 360) * Math.cos((cPt[1] * Math.PI) / 180));
      ...
      const safeElecPoly =
        turf.intersect(elecBoxFeature, workingPoly) || elecBoxFeature;
      ...
      try {
        ...
        console.warn(
          "[Layout Generator] Failed to merge electrical shaft with core geometry",
          e,
        );
      }

      console.log(
        "[Layout Generator] Placed Electrical Shaft at SE corner, merged into core area",
      );
    } catch (e) {
  ...
  if (!isPodiumBuilding && shouldInclude("HVAC")) {
    try {
      ...
      const startPoint = turf.transformTranslate(swPoint, 1, 45, {
        units: "meters",
      });
      ...
      const hvacBoxFeature = turf.bboxPolygon([
        startCoords[0],
        startCoords[1],
        startCoords[0] + hvacWDeg,
        startCoords[1] + hvacDDeg,
      ]);
      ...
      if (!hvacPoly || turf.area(hvacPoly) < 4) {
        ...
        const fallbackCenter = turf.transformTranslate(
          center,
          width * 0.15,
          -90,
          { units: "meters" },
        ); // Shift west
        hvacPoly = turf.envelope(
          turf.buffer(fallbackCenter, hvacSize / 2, { units: "meters" }),
        );
        // @ts-ignore
      ...
      console.log(
        "[Layout Generator] Placed Rooftop HVAC Unit (W/SW - Vastu compliant)",
      );
    } catch (e) {
  ...
  if (
    !isPodiumBuilding &&
    (shouldInclude("Solar PV") || shouldInclude("Rooftop Solar"))
  ) {
    try {
      const bBox = turf.bbox(workingPoly);
      ...
      const solarStart = turf.transformTranslate(sePoint, 1, -135, {
        units: "meters",
      });
      ...
      const solarBoxFeature = turf.bboxPolygon([
        solarCoords[0] - solarWDeg,
        solarCoords[1],
        solarCoords[0],
        solarCoords[1] + solarDDeg,
      ]);
      ...
      if (!solarPoly || turf.area(solarPoly) < 4) {
        ...
        solarPoly = turf.envelope(
          turf.buffer(fallbackCenter, solarSize / 2, { units: "meters" }),
        );
      ...
      console.log(
        `[Layout Generator] Placed Rooftop Solar PV (S/SE - ${solarArea.toFixed(1)}m²)`,
      );
    } catch (e) {
  ...
  if (!isTowerBuilding && shouldInclude("EV Charging")) {
    try {
      ...
      const evStart = turf.transformTranslate(nwPoint, 0.5, 135, {
        units: "meters",
      });
  ...
  if (coreGeom) {
    if (params.exactTypologyAllocation) {
      ...
      const coreWithCorridor = turf.buffer(coreGeom, corridorW / 1000, {
        units: "kilometers",
      });
  ...
  if (obstacles) {
    try {
      ...
      try {
        // @ts-ignore
        leasablePoly = turf.difference(
          workingPoly,
          turf.buffer(obstacles, 0.001, { units: "kilometers" }),
        );
      } catch (err2) {
        console.warn(
          "Buffer fallback also failed for difference. Using original polygon.",
          err2,
        );
      }
    }
  ...
  if (params.intendedUse === BuildingIntendedUse.Retail) {
    useUnitMix = [{ name: "Retail Unit", mixRatio: 1, area: 100 }];
  } else if (params.intendedUse === BuildingIntendedUse.Office) {
    useUnitMix = [{ name: "Office", mixRatio: 1, area: 80 }];
  } else if (params.intendedUse === BuildingIntendedUse.Commercial) {
    ...
    params.intendedUse === BuildingIntendedUse.Public ||
    params.intendedUse === BuildingIntendedUse.Industrial
  ) {
    useUnitMix = [{ name: "Hall", mixRatio: 1, area: 500 }];
  } else if (!useUnitMix || useUnitMix.length === 0) {
    useUnitMix = [
      { name: "2BHK", mixRatio: 0.3, area: 140 },
      { name: "3BHK", mixRatio: 0.35, area: 185 },
      { name: "4BHK", mixRatio: 0.35, area: 245 },
    ];
  }

  const weightedAvgSize = useUnitMix.reduce(
    (acc, unit) => acc + unit.area * unit.mixRatio,
    0,
  );
  ...
  gridFeatures.forEach((cell) => {
    try {
      ...
      if (intersection && turf.area(intersection) > actualCellAreaM2 * 0.02) {
        validModules.push({
          ...cell,
          validPoly: intersection as Feature<Polygon>,
        });
      }
    } catch (e) {}
  });
  ...
  const leasableAreaM2 = validModules.reduce(
    (sum, m) => sum + turf.area(m.validPoly),
    0,
  );
  console.log(
    `[Layout Generator] Tiled Grid: ${totalValidCells} modules, cell≈${actualCellAreaM2.toFixed(1)}m², leasable≈${leasableAreaM2.toFixed(0)}m²`,
  );
  ...
  if (isHorizontalBuilding) {
    stripA = {
      ...
      maxY: lMaxY,
      depth: turf.distance(
        turf.point([lMinX, coreCenter[1]]),
        turf.point([lMinX, lMaxY]),
        { units: "meters" },
      ),
      length: width,
    };
    ...
    stripA = {
      ...
      maxY: lMaxY,
      depth: turf.distance(
        turf.point([coreCenter[0], lMinY]),
        turf.point([lMaxX, lMinY]),
        { units: "meters" },
      ),
      length: depth,
    };
  ...
  const totalStripArea =
    stripA.depth * stripA.length + stripB.depth * stripB.length;

  console.log(
    `[Layout Generator] Strips: A depth=${stripA.depth.toFixed(1)}m len=${stripA.length.toFixed(1)}m (${(stripA.depth * stripA.length).toFixed(0)}m²), B depth=${stripB.depth.toFixed(1)}m len=${stripB.length.toFixed(1)}m (${(stripB.depth * stripB.length).toFixed(0)}m²)`,
  ...
  const computeUnitWidths = (stripDepth: number) => {
    if (stripDepth < 1)
      return useUnitMix.map((u) => ({ ...u, unitWidth: u.area }));
  ...
  const avgUnitWidth = unitWidths.reduce(
    (s, u) => s + u.unitWidth * u.mixRatio,
    0,
  );
  const totalLength = stripA.length;

  const totalUnitSlots = Math.max(
    1,
    Math.floor((totalLength * 2) / avgUnitWidth),
  );
  ...
  let totalWidthUsed = unitCounts.reduce(
    (s, u) => s + u.count * u.unitWidth,
    0,
  );
  ...
  console.log(
    `[Layout Generator] Floor plan:`,
    unitCounts
      .map((u) => `${u.name}=${u.count}×${u.unitWidth.toFixed(1)}m`)
      .join(", "),
  ...
  if (params.exactTypologyAllocation && allocBudget > 0) {
    ...
    // Sort mix types descending by area: 4BHK > 3BHK > 2BHK
    const sortedMixTypes = [...mixTypes].sort((a, b) => b.area - a.area);
    const sortedMaxCounts = sortedMixTypes.map((u) =>
      Math.floor(allocBudget / u.area),
    );
    ...
    const enumerate = (
      ...
      if (typeIdx >= sortedMixTypes.length) {
        const uniqueTypes = Object.values(currentCounts).filter(
          (c) => c > 0,
        ).length;
        ...
        if (
          ...
          (currentTotal === bestTotal && uniqueTypes > bestUniqueTypes)
        ) {
      ...
      const ut = sortedMixTypes[typeIdx];
      const maxForThis = Math.min(
        sortedMaxCounts[typeIdx],
        Math.floor(remainingArea / ut.area),
      );

      // Start from maxForThis DOWN to 0 to heavily favor allocating larger units first (when sorted by area DESC)
      // This achieves the "dynamic proportion 4bhk > 3bhk > 2bhk" effect, filling as much as possible.
      for (let c = maxForThis; c >= 0; c--) {
        currentCounts[ut.name] = c;
        enumerate(
          typeIdx + 1,
          remainingArea - c * ut.area,
          currentCounts,
          currentTotal + c * ut.area,
        );
      }
    ...
    const utilization =
      allocBudget > 0 ? ((bestTotal / allocBudget) * 100).toFixed(1) : "0";
    console.log(
      `[Layout Generator] [exactTypology] Optimal packing: sum=${bestTotal.toFixed(0)}m² / stripBudget=${allocBudget.toFixed(0)}m² (${utilization}%) | counts:`,
  ...
  const remaining = unitCounts.map((u) => ({
    ...u,
    left: u.count,
    colorIdx: 0,
  }));
  ...
  while (left > 0) {
    for (const u of remaining) {
      if (u.left > 0) {
        const baseColor = getColorForUnitType(u.name);
        const color =
          u.colorIdx % 3 === 0
            ? baseColor
            : u.colorIdx % 3 === 1
  ...
  if (params.exactTypologyAllocation) {
    ...
    const buildingSeed = Math.floor(
      coreCenter[0] * 100000 + coreCenter[1] * 100000,
    );
  ...
  const createStripUnits = (
    ...
    if (deck.length === 0 || strip.depth < 1 || strip.length < 1)
      return { nextIdx: unitStartIdx, spillover: deck };
    const exactDeckWidth = deck.reduce(
      (s, u) => s + u.targetArea / strip.depth,
      0,
    );
    const scaleFactor = exactDeckWidth > 0 ? strip.length / exactDeckWidth : 1;
    ...
    for (let d = 0; d < deck.length; d++) {
      ...
      try {
        ...
        if (clipped) {
          if (clipped.geometry.type === "Polygon") {
            ...
            const parts = (clipped.geometry.coordinates as any[]).map(
              (c: any) => turf.polygon(c),
            );
      ...
      if (finalGeom && turf.area(finalGeom) > 5) {
        ...
        const baseColor = getColorForUnitType(assignment.type);
        const finalColor =
          idx % 3 === 0
            ? baseColor
            : idx % 3 === 1
  ...
  console.log(
    `[Layout Generator] Final: ${units.length} units placed.`,
  ...
  // --- GROUND FLOOR ROAD-SIDE REDUCTION ---
  ...
  if (params.groundFloorRoadSideReduction && units.length > 0) {
    try {
      ...
      uBboxes.forEach(b => {
        ...
        minU_Y = Math.min(minU_Y, b[1]);
        maxU_X = Math.max(maxU_X, b[2]);
      ...
      console.warn("[Layout Gen] Ground floor reduction failed, keeping all units:", e);
      groundFloorUnits = units;
    }
  }
  ...
  if (rotationAngle !== 0) {
    cores.forEach((c) => {
      if (c.geometry) {
        // @ts-ignore
        c.geometry = turf.transformRotate(c.geometry, rotationAngle, {
          pivot: center,
        });
      }
    });
    utilities.forEach((u) => {
      if (u.geometry) {
        // @ts-ignore
        u.geometry = turf.transformRotate(u.geometry, rotationAngle, {
          pivot: center,
        });
    ...
    // groundFloorUnits are a subset of units — rotate their geometries back too.
    // Since they share the same objects as units[], they're already rotated above.
    // We only need to handle the case where groundFloorUnits !== units (no rotation happened yet).
    entrances.forEach((e) => {
      if (e.position && e.position.length >= 2) {
        const pt = turf.point([e.position[0], e.position[1]]);
        // @ts-ignore
        const rotatedPt = turf.transformRotate(pt, rotationAngle, {
          pivot: center,
        });
    ...
    console.log(
      `[Layout Gen] Master rotated all components back by ${rotationAngle}deg`,
    );
  }
...
// ══════════════════════════════════════════════════════════════════════════════
// COMMERCIAL LAYOUT GENERATORS — Retail & Office
// Based on Logic & Methodology document benchmarks:
//   Retail: Core+Circ = 35-50%, corner core, central corridor, retail units
//   Office: Core+Circ = 25-35%, center core, circulation ring, offices on all sides
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Generates a RETAIL floor layout:
 *   ┌──────────────────┬──────┐
 *   │   R   R   R   R  │      │
 *   ├──────────────────┤ CORE │
 *   │   CIRCULATION    │      │
 ...
 *   │   R   R   R   R  │      │
 *   └──────────────────┴──────┘
 * Core at corner, circulation strip in the middle, retail units on both sides.
 */
function generateRetailLayout(
  buildingPoly: Feature<Polygon | MultiPolygon>,
  ...
  console.log("[Layout Generator] Generating RETAIL layout");
  ...
  // Population: Commercial = A / 10 sqm per person (NBC)
  const popPerFloor = floorArea / 10;

  // --- CORE SIZING (from methodology doc) ---
  // Lifts: 1 per 250-300 occupants OR 1 per 800-1000 sqm
  let passengerLifts = Math.max(1, Math.ceil(popPerFloor / 275));
  const areaBasedLifts = Math.max(1, Math.ceil(floorArea / 900));
  passengerLifts = Math.max(passengerLifts, areaBasedLifts);
  let serviceLifts = 0;
  let fireLift = 0;
  ...
  if (floors > 20) serviceLifts = 1;
  const totalLifts = passengerLifts + serviceLifts + fireLift;
  const liftArea = passengerLifts * 2.5 + serviceLifts * 5.0 + fireLift * 4.5;

  // Stairs: 18-25 sqm per stair (commercial)
  let stairCount = 2;
  if (popPerFloor > 500) stairCount = 3;
  const stairArea = stairCount * 22;

  // Lobby: 5-7 sqm per lift
  const lobbyArea = totalLifts * 6;

  // Corridor: Retail gets 8-10% (higher for customer flow)
  const corridorArea = floorArea * 0.09;

  // Shafts
  const plumbingShafts = Math.max(1, Math.ceil(floorArea / 750));
  const shaftArea = plumbingShafts * 0.85 + 0.6 + 0.5 + Math.ceil(floorArea / 1000) * 3;
  ...
  // Benchmark Enforcement: Commercial Core should be 16-22% of BUA (Methodology Doc)
  let enforcedCoreArea = totalCoreArea;
  if ((totalCoreArea / floorArea) < 0.16) {
     console.log(`[Retail Layout] Calculated physical core (${totalCoreArea.toFixed(0)}m²) is below 16% benchmark. Enforcing minimum 16%.`);
     enforcedCoreArea = floorArea * 0.16;
  }

  // Target: Core+Circulation should be 35-50% of floor area for retail
  const coreCircTarget = Math.max(enforcedCoreArea + corridorArea, floorArea * 0.35);

  console.log(
    `[Retail Layout] Floor=${floorArea.toFixed(0)}m², Enforced Core=${enforcedCoreArea.toFixed(0)}m², Corridor=${corridorArea.toFixed(0)}m², Total=${coreCircTarget.toFixed(0)}m² (${((coreCircTarget / floorArea) * 100).toFixed(1)}%)`,
  );
  ...
  const longAxis = isHorizontal ? bboxWidth : bboxDepth;
  const shortAxis = isHorizontal ? bboxDepth : bboxWidth;

  const degPerMeterX = (maxX - minX) / bboxWidth;
  const degPerMeterY = (maxY - minY) / bboxDepth;

  // --- CORE PLACEMENT: CORNER ---
  // Slice the building from the edge to get exactly totalCoreArea
  ...
  for (let i = 0; i < 20; i++) {
    ...
    try {
      ...
      if (Math.abs(a - enforcedCoreArea) < enforcedCoreArea * 0.05) {
        break;
      }
      
      if (a > enforcedCoreArea) {
        // Sliced too much area, move cut line closer to the edge (maxX / maxY)
        lo = mid;
      } else {
        // Sliced too little area, move cut line deeper into the building (minX / minY)
        hi = mid;
      }
    } catch(e) {
      break;
  ...
  if (clippedCore && turf.area(clippedCore) > 10) {
    cores.push({
      id: "core-retail-corner",
      type: "Lobby",
      geometry: clippedCore as Feature<Polygon>,
    });
    console.log(`[Retail Layout] Core placed at corner: ${turf.area(clippedCore).toFixed(0)}m² (Target: ${enforcedCoreArea.toFixed(0)}m²)`);
  }
  ...
  if (clippedCore) {
    try {
      ...
      console.warn("[Retail Layout] Core subtraction failed, using full building");
    }
  }

  // --- CIRCULATION: HORIZONTAL STRIP IN THE MIDDLE ---
  ...
  let stripB_bounds: { minX: number; maxX: number; minY: number; maxY: number };

  if (isHorizontal) {
    // Horizontal building: circulation runs E-W in the middle of depth
    ...
    // Strip A: above circulation
    stripA_bounds = { minX: lMinX, maxX: lMaxX, minY: midY + halfCircDeg, maxY: lMaxY };
    // Strip B: below circulation
    stripB_bounds = { minX: lMinX, maxX: lMaxX, minY: lMinY, maxY: midY - halfCircDeg };
  } else {
    // Vertical building: circulation runs N-S in the middle of width
    ...
    // Strip A: right of circulation
    stripA_bounds = { minX: midX + halfCircDeg, maxX: lMaxX, minY: lMinY, maxY: lMaxY };
    // Strip B: left of circulation
    stripB_bounds = { minX: lMinX, maxX: midX - halfCircDeg, minY: lMinY, maxY: lMaxY };
  }
  ...
  if (clippedCirc) {
    try {
      ...
      console.warn("[Retail Layout] Circulation subtraction failed");
    }
  }
...
/**
 * Generates an OFFICE floor layout:
 *   ┌──────┬──────┬──────┬──────┐
 *   │  OF  │  OF  │  OF  │  OF  │
 *   ├──────┼──────┴──────┼──────┤
 *   │  OF  │ CIRCULATION │  OF  │
 ...
 *   │  OF  │  OF  │  OF  │  OF  │
 *   └──────┴──────┴──────┴──────┘
 * Core at center, circulation ring wrapping core, offices on all 4 sides.
 */
function generateOfficeLayout(
  ...
  console.log("[Layout Generator] Generating OFFICE layout");
  ...
  let enforcedCoreArea = totalCoreArea;
  if ((totalCoreArea / floorArea) < 0.16) {
     console.log(`[Office Layout] Calculated physical core (${totalCoreArea.toFixed(0)}m²) is below 16% benchmark. Enforcing minimum 16%.`);
     enforcedCoreArea = floorArea * 0.16;
  }

  // Target: Core+Circulation = 25-35% for office
  const corridorArea = floorArea * 0.07; // 6-8%
  const coreCircTarget = Math.max(enforcedCoreArea + corridorArea, floorArea * 0.25);

  console.log(
    `[Office Layout] Floor=${floorArea.toFixed(0)}m², Enforced Core=${enforcedCoreArea.toFixed(0)}m², Corridor=${corridorArea.toFixed(0)}m², Total=${coreCircTarget.toFixed(0)}m² (${((coreCircTarget / floorArea) * 100).toFixed(1)}%)`,
  );
  ...
  // Core occupies center ~20% of the footprint
  const coreRatio = Math.min(0.25, Math.max(0.15, enforcedCoreArea / floorArea));
  const coreSide = Math.sqrt(enforcedCoreArea);
  const coreW = Math.min(coreSide, bboxWidth * 0.35);
  const coreH = Math.min(enforcedCoreArea / coreW, bboxDepth * 0.35);

  const midX = (minX + maxX) / 2;
  const midY = (minY + maxY) / 2;
  const halfCoreWDeg = (coreW / 2) * degPerMeterX;
  const halfCoreHDeg = (coreH / 2) * degPerMeterY;

  const corePoly = turf.bboxPolygon([
    midX - halfCoreWDeg,
    midY - halfCoreHDeg,
    midX + halfCoreWDeg,
    midY + halfCoreHDeg,
  ...
  // Clip core to building
  let clippedCore = turf.intersect(corePoly, workingPoly);
  if (clippedCore && turf.area(clippedCore) > 10) {
    cores.push({
      id: "core-office-center",
      type: "Lobby",
      geometry: clippedCore as Feature<Polygon>,
    });
    console.log(`[Office Layout] Core placed at center: ${turf.area(clippedCore).toFixed(0)}m²`);
  } else {
    // Fallback: use centroid
    const centerPt = turf.centroid(workingPoly);
    const fallbackCore = turf.bboxPolygon([
      centerPt.geometry.coordinates[0] - halfCoreWDeg,
      centerPt.geometry.coordinates[1] - halfCoreHDeg,
      centerPt.geometry.coordinates[0] + halfCoreWDeg,
      centerPt.geometry.coordinates[1] + halfCoreHDeg,
    ]);
    clippedCore = turf.intersect(fallbackCore, workingPoly);
    if (clippedCore) {
      cores.push({
        id: "core-office-center",
        type: "Lobby",
  ...
  // Corridor width mathematically derived to hit the remaining coreCircTarget
  const targetCircArea = Math.max(0, coreCircTarget - enforcedCoreArea);
  // Area of buffer ~ perimeter * width
  const perimeter = 2 * (coreW + coreH);
  let corridorWidthM = 2.5; // fallback minimum
  if (perimeter > 0) {
      corridorWidthM = Math.max(2.5, targetCircArea / perimeter);
  }
  console.log(`[Office Layout] Corridor ring generated at ${corridorWidthM.toFixed(1)}m wide to fulfill ${targetCircArea.toFixed(0)}m² benchmark target.`);

  let coreWithCirc: Feature<Polygon | MultiPolygon> | null = null;
  if (clippedCore) {
    try {
  ...
  // --- LEASABLE AREA: building minus (core + circulation ring) ---
  let leasablePoly = workingPoly;
  const obstacle = coreWithCirc || clippedCore;
  if (obstacle) {
    try {
      // @ts-ignore
      leasablePoly = turf.difference(workingPoly, obstacle) || workingPoly;
    } catch (e) {
      console.warn("[Office Layout] Core+circ subtraction failed, using full building");
    }
  }
...
export function calculateUtilityReservationZones(
  ...
  console.log(
    `[Utility Reservation] Created ${reservationZones.length} corner zones for Vastu utilities`,
  );
...
export function generateSiteUtilities(
  ...
  buildings.forEach((b) => {
    if (
      b.properties &&
      b.properties.units &&
      Array.isArray(b.properties.units)
    ) {
      ...
      try {
        ...
        if (area > 0) {
          const floors =
            b.numFloors || (b.properties && b.properties.floors) || 5;
        ...
        console.warn(
          "[Utility Generator] Error calculating building area for unit estimate:",
          e,
        );
      }
    }
  ...
  const avgFloors =
    buildings.length > 0
      ? Math.round(
          buildings.reduce((s: number, b: any) => s + (b.numFloors || 1), 0) /
            buildings.length,
        )
      : 5;
  const hydrantCount = Math.ceil(avgFloors / 5) * 2;
  const fireTankVolume =
    (Math.max(hydrantCount * 40, (((300 * 5) / 1000) * 1000) / 60) * 30) / 1000; // 30min storage
  ...
  console.log(
    `Units: ${totalUnits}, Pop: ${population} (${avgPersonsPerUnit}/unit)`,
  );
  console.log(
    `STP -> Cap: ${stpCapacityKLD.toFixed(1)} KLD | Area: ${stpArea} sqm (@8 m²/KLD)`,
  );
  console.log(
    `WTP -> Cap: ${wtpCapacityKLD.toFixed(1)} KLD | Area: ${wtpArea} sqm (@6 m²/KLD)`,
  );
  console.log(
    `UGT (Water) -> Vol: ${ugtVolume.toFixed(1)} m³ | Area: ${ugtArea} sqm (2.5m depth)`,
  );
  console.log(
    `RWH -> Roof: ${roofAreaSum.toFixed(1)} m² | Harvest: ${annualHarvestVol.toFixed(1)} m³ | Storage: ${rwhVolume.toFixed(1)} m³ | Area: ${rwhArea} sqm (2.5m depth)`,
  );
  console.log(
    `Electrical -> Peak: ${totalLoadKW.toFixed(0)} kW | Essential: ${essentialLoad.toFixed(0)} kW | DG: ${dgKVA.toFixed(0)} kVA`,
  );
  console.log(`DG Set Area -> ${dgArea} sqm (25m²/500kVA)`);
  console.log(
    `Substation -> ${substationArea} sqm | Transformer -> ${transformerArea} sqm`,
  );
  console.log(
    `Fire Tank -> Vol: ${fireTankVolume.toFixed(0)} m³ | Area: ${fireTankArea} sqm (3.0m depth)`,
  );
  console.log(
    `Gas: ${gasArea} sqm | Admin: ${adminBlockArea} sqm | OWC: ${owcArea} sqm`,
  );
  console.log(
    `Solar PV -> ${solarCapacityKW.toFixed(1)} kW = ${solarAreaReq.toFixed(1)} sqm`,
  );
  ...
  const shouldInclude = (type: string) =>
    !selectedUtilities || selectedUtilities.includes(type);

  // NE: Water Zone (Vastu: Water/Eshanya)
  if (shouldInclude(UtilityType.Water))
    groupNE.push({
  ...
  if (shouldInclude(UtilityType.RainwaterHarvesting))
    groupNE.push({
      type: UtilityType.RainwaterHarvesting,
      name: "RWH Tank",
      area: rwhArea,
      color: "#81D4FA",
      height: 2.5,
  ...
  if (shouldInclude(UtilityType.Electrical))
    groupSE.push({
      type: UtilityType.Electrical,
      name: "Substation/Transf.",
      area: transformerArea + substationArea,
      color: "#FF9800",
      height: 2.5,
      level: 0,
    });
  if (shouldInclude(UtilityType.DGSet))
    groupSE.push({
      type: UtilityType.DGSet,
      name: "DG Set",
      area: dgArea,
      color: "#FFB74D",
      height: 2.5,
  ...
  if (shouldInclude(UtilityType.Gas))
    groupSE.push({
  ...
  if (shouldInclude(UtilityType.Fire))
    groupSE.push({
      type: UtilityType.Fire,
      name: "Fire Pump Room",
      area: firePumpRoomArea,
      color: "#FF5722",
      height: 2.5,
      level: -1,
    });
  ...
  if (shouldInclude(UtilityType.STP))
    groupNW.push({
      type: UtilityType.STP,
      name: "STP Plant",
      area: stpArea,
      color: "#BA68C8",
      height: 2.5,
  ...
  if (shouldInclude(UtilityType.SolidWaste))
    groupNW.push({
      type: UtilityType.SolidWaste,
      name: "OWC (Waste)",
      area: owcArea,
      color: "#8D6E63",
      height: 2,
  ...
  if (shouldInclude(UtilityType.WTP))
    groupNW.push({
      type: UtilityType.WTP,
      name: "WTP Plant",
      area: wtpArea,
      color: "#29B6F6",
      height: 3,
  ...
  if (shouldInclude(UtilityType.Admin))
    groupSW.push({
      type: UtilityType.Admin,
      name: "Admin / Security",
      area: adminBlockArea,
      color: "#FDD835",
      height: 3,
  ...
  const placeGroupInCorner = (
    ...
    console.log(
      `[Utility Debug] Placing in ${corner}: bbox=[${minX}, ${minY}, ${maxX}, ${maxY}], corner=[${cornerX}, ${cornerY}]`,
    );
    ...
    // @ts-ignore
    while (
      !turf.booleanPointInPolygon(currentPoint, containerPoly) &&
      steps < maxSteps
    ) {
      const bearing = turf.bearing(currentPoint, marchTarget);
      currentPoint = turf.destination(currentPoint, 1, bearing, {
        units: "meters",
      });
      steps++;
    }
    ...
    if (steps >= maxSteps) {
      console.warn(
        `[Utility Debug] Could not find valid start point for corner ${corner}`,
      );
      return group;
    }
    ...
    const bearing = turf.bearing(currentPoint, marchTarget);
    currentPoint = turf.destination(currentPoint, effectiveMargin, bearing, {
      units: "meters",
    });
    ...
    group.forEach((util) => {
      ...
      for (const pt of gridPoints) {
        ...
        let uSeed = 0;
        for (let c = 0; c < uSeedStr.length; c++)
          uSeed += uSeedStr.charCodeAt(c);

        const shapeTypeInt = uSeed % 4;
        let poly: Feature<Polygon>;

        if (shapeTypeInt === 0) {
          poly = createRotatedRect(
            turf.point([originX, originY]),
            uW,
            uH,
            plotOrientation,
          );
          ...
          poly = turf.circle(turf.point([originX, originY]), radius, {
            units: "meters",
            steps: steps,
          });
        }
        ...
        try {
          if (peripheralParkingZone) {
            ...
            // @ts-ignore
            const safeContainer =
              turf.buffer(containerPoly, -1.0, { units: "meters" }) ||
              containerPoly;
            // @ts-ignore
        ...
        for (const obst of obstacles) {
          try {
            if (obst && obst.geometry) {
              // @ts-ignore
              if (turf.booleanIntersects(checkPoly, obst.geometry)) {
        ...
        for (const b of buildings) {
          try {
            if (b && b.geometry && b.visible !== false) {
              // @ts-ignore
              if (turf.booleanIntersects(checkPoly, b.geometry)) {
        ...
        if (buildingOverlap) {
          const colliding = buildings.filter(
            (b) =>
              b.visible !== false &&
              turf.booleanIntersects(checkPoly, b.geometry),
          );
          ...
          for (const b of colliding) {
            ...
            try {
              ...
              if (w > 20 && h > 20) {
                // Only shrink if reasonable size
                ...
                for (const s of scales) {
                  ...
                  if (isValidCandidate(scaled.geometry)) {
                    ...
                    console.log(
                      `[Utility Force] Shrunk building ${b.id} by factor ${s} to fit`,
                    );
                    resolved = true;
                    break;
                  }
            ...
            b.visible = false;
            console.log(
              `[Utility Force] Removing building ${b.id} (Resize failed)`,
            );
          }
        ...
        try {
          ...
          bufferedCandidate = poly;
        }

        for (const u of utilities) {
          try {
            if (turf.booleanIntersects(poly, u.geometry)) {
              utilityOverlap = true;
              break;
            }
            if (
              bufferedCandidate &&
              turf.booleanIntersects(bufferedCandidate, u.geometry)
            ) {
              utilityOverlap = true;
              break;
            }
        ...
        console.log(
          `[Utility Debug] ✓ Placed ${util.name} in ${corner} (Grid ${pt.i},${pt.j})`,
        );
        placed = true;
      ...
      if (!placed) {
        console.warn(
          `[Utility] Could not place ${util.name} in ${corner} after grid search.`,
        );
  ...
  if (vastuCompliant) {
    ...
    failedNW.forEach((u) => {
      if (
        ["STP", "WTP", "OWC"].some(
          (t) => u.type.includes(t) || u.name.includes(t),
        )
      ) {
        console.log(
          `[Utility Failover] Moving ${u.name} from NW to SE (Acceptable Direction)`,
        );
        retrySE.push(u);
      }
    });
    ...
    if (remaining.length > 0) {
      console.warn(
        `[Utility] ${remaining.length} utilities could not be placed in any corner:`,
        remaining.map((u) => u.name).join(", "),
      );
    }
  }
...
export function generateSiteGates(
  ...
  let sides =
    roadAccessSides.length > 0 ? roadAccessSides : ["N", "S", "E", "W"];
  console.log(
    `[Gates] Using road access sides: ${sides.join(", ")} (auto=${roadAccessSides.length === 0})`,
  );
  ...
  const findClosestBoundaryPoint = (
    targetLng: number,
    targetLat: number,
  ): [number, number] => {
  ...
  const isColliding = (point: [number, number]): boolean => {
    ...
    return existingBuildings.some((b) => {
      // @ts-ignore
      return (
        turf.booleanOverlap(buffer, b.geometry) ||
        turf.booleanContains(b.geometry, pt) ||
        turf.booleanPointInPolygon(pt, b.geometry)
      );
    });
  };
  ...
  const findValidPosition = (
    ...
    // @ts-ignore
    const startDist = turf.nearestPointOnLine(plotLine as any, startPt)
      .properties.location;
    ...
    console.warn(
      "[Gates] Could not find non-colliding position for gate, defaulting to collision",
    );
    return bestPos;
  };
  ...
  if (vastuCompliant) {
    console.log(
      `[Gate Generator] Vastu mode enabled. Sides: ${sides.join(", ")}`,
    );
    const center: [number, number] = [cx, cy];
    const newGates = generateVastuGates(
      plotPoly as Feature<Polygon>,
      center,
      sides,
    );
